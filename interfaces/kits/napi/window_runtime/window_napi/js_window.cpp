...
napi_value JsWindow::OnGetProperties(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), errCode, env, task = napiAsyncTask] {
        ...
        if (window == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getProperties", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getProperties", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode));
...
napi_value JsWindow::OnRegisterWindowCallback(napi_env env, napi_callback_info info)
{
    ...
    if (!NapiIsCallable(env, callback)) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.on",
            WmErrorCode::WM_ERROR_INVALID_PARAM);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM, "[window][on]msg: Callback is not callable.");
    }
...
napi_value JsWindow::OnSetFullScreen(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), isFullScreen, errCode, where,
        ...
        if (weakWindow == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setFullScreen", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setFullScreen", errCode);
        ...
        if (ret == WMError::WM_OK) {
            ...
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setFullScreen", ret);
...
napi_value JsWindow::OnSetLayoutFullScreen(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), isLayoutFullScreen, errCode, where,
        ...
        if (weakWindow == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setLayoutFullScreen", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setLayoutFullScreen", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode, "Invalidate params."));
            return;
        }
        ...
        if (ret == WMError::WM_OK) {
            ...
            TLOGNE(WmsLogTag::WMS_IMMS, "%{public}s failed, ret %{public}d", where, ret);
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setLayoutFullScreen", ret);
...
napi_value JsWindow::OnSetSystemBarEnable(napi_env env, napi_callback_info info)
{
    ...
    if (argc > ARG_COUNT_TWO || !GetSystemBarStatus(env, info, systemBarProperties, systemBarPropertyFlags)) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSystemBarEnable", WMError::WM_ERROR_INVALID_PARAM);
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), env, task = napiAsyncTask,
        ...
        if (window == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSystemBarEnable", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        ...
        if (errCode == WMError::WM_OK) {
            ...
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSystemBarEnable", errCode);
...
napi_value JsWindow::OnSetWindowSystemBarEnable(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), env, task = napiAsyncTask,
        ...
        if (window == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowSystemBarEnable", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
            task->Reject(env, JsErrUtils::CreateJsError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
napi_value JsWindow::OnSetSpecificSystemBarEnabled(napi_env env, napi_callback_info info)
{
    ...
    if (!ConvertFromJsValue(env, argv[INDEX_ZERO], name) ||
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSpecificSystemBarEnabled", WmErrorCode::WM_ERROR_INVALID_PARAM);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM,
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), env, task = napiAsyncTask,
        ...
        if (window == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "%{public}s window is nullptr", where);
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSpecificSystemBarEnabled", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
            task->Reject(env, JsErrUtils::CreateJsError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
napi_value JsWindow::OnSetSystemBarProperties(napi_env env, napi_callback_info info)
{
    ...
    if (argc < ARG_COUNT_ONE || argc > ARG_COUNT_TWO || argv[INDEX_ZERO] == nullptr ||
        ...
        TLOGE(WmsLogTag::WMS_IMMS, "Failed to convert parameter to systemBarProperties");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSystemBarProperties", WMError::WM_ERROR_INVALID_PARAM);
        napiAsyncTask->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_INVALID_PARAM));
        return result;
    }
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), env, task = napiAsyncTask,
        ...
        if (window == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSystemBarProperties", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        auto errCode = window->UpdateSystemBarProperties(systemBarProperties, systemBarPropertyFlags);
        if (errCode == WMError::WM_OK) {
            ...
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setSystemBarProperties", errCode);
...
napi_value JsWindow::OnSetWindowSystemBarProperties(napi_env env, napi_callback_info info)
{
    ...
    if (argc < ARG_COUNT_ONE) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowSystemBarProperties", WmErrorCode::WM_ERROR_INVALID_PARAM);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM,
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), env, task = napiAsyncTask,
        ...
        if (window == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowSystemBarProperties", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
            task->Reject(env, JsErrUtils::CreateJsError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
napi_value JsWindow::OnGetWindowSystemBarPropertiesSync(napi_env env, napi_callback_info info)
{
    if (windowToken_ == nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getWindowSystemBarProperties", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
    ...
    if (!WindowHelper::IsMainWindow(windowToken_->GetType())) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getWindowSystemBarProperties", WmErrorCode::WM_ERROR_INVALID_CALLING);
    ...
    if (objValue == nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getWindowSystemBarProperties", WmErrorCode::WM_ERROR_SYSTEM_ABNORMALLY);
...
napi_value JsWindow::OnGetAvoidArea(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), errCode, avoidAreaType, where,
        ...
        if (weakWindow == nullptr) {
            TLOGNE(WmsLogTag::WMS_IMMS, "%{public}s window is nullptr", where);
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getAvoidArea", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
...
napi_value JsWindow::OnGetWindowAvoidAreaIgnoringVisibilitySync(napi_env env, napi_callback_info info)
{
    ...
    if (argc < 1) { // 1: params num
        TLOGE(WmsLogTag::WMS_IMMS, "invalid argc %{public}zu", argc);
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getWindowAvoidAreaIgnoringVisibility", WmErrorCode::WM_ERROR_INVALID_PARAM);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM,
    ...
    if (windowToken_ == nullptr) {
        TLOGE(WmsLogTag::WMS_IMMS, "window is nullptr");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getWindowAvoidAreaIgnoringVisibility", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
napi_value JsWindow::OnIsSupportWideGamut(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), errCode, where, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.isSupportWideGamut", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.isSupportWideGamut", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode));
            WLOGFE("window is nullptr or get invalid param");
            return;
        }
...
napi_value JsWindow::OnIsWindowSupportWideGamut(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), where, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr or get invalid param");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.isWindowSupportWideGamut", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
            task->Reject(env,
                JsErrUtils::CreateJsError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
napi_value JsWindow::OnSetBackgroundColor(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), color, errCode, where, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setBackgroundColor", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR,
                "[window][setBackgroundColor]msg: invalid window"));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setBackgroundColor", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode,
        ...
        if (ret == WMError::WM_OK) {
            ...
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setBackgroundColor", ret);
            task->Reject(env, JsErrUtils::CreateJsError(env, ret,
...
napi_value JsWindow::OnSetWindowBackgroundColorSync(napi_env env, napi_callback_info info)
{
    ...
    if (windowToken_ == nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowBackgroundColor", WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        return NapiThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
napi_value JsWindow::OnSetBrightness(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), brightness, errCode, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setBrightness", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setBrightness", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode, "Invalidate params."));
            return;
        }
        ...
        if (ret == WMError::WM_OK) {
            ...
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setBrightness", ret);
...
napi_value JsWindow::OnSetPrivacyMode(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), isPrivacyMode, errCode, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setPrivacyMode", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR,
                "[window][setPrivacyMode]msg: invalid window"));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setPrivacyMode", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode,
...
napi_value JsWindow::OnSetColorSpace(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), colorSpace, errCode, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setColorSpace", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setColorSpace", errCode);
...
napi_value JsWindow::OnGetColorSpace(napi_env env, napi_callback_info info)
{
    ...
    auto asyncTask = [weakToken = wptr<Window>(windowToken_), errCode, where, env, task = napiAsyncTask] {
        ...
        if (weakWindow == nullptr) {
            WLOGFE("window is nullptr");
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getColorSpace", WMError::WM_ERROR_NULLPTR);
            task->Reject(env, JsErrUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR));
            return;
        }
        if (errCode != WMError::WM_OK) {
            HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getColorSpace", errCode);
            task->Reject(env, JsErrUtils::CreateJsError(env, errCode));
            WLOGFE("window is nullptr or get invalid param");
            return;
        }
