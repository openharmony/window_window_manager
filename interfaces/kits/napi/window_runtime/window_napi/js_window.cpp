...
napi_value JsWindow::OnRegisterWindowCallback(napi_env env, napi_callback_info info)
{
    if (windowToken_ == nullptr) {
        WLOGFE("Window is nullptr");
        return NapiThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY, "[window][on]msg: invalid window");
    }
    ...
    if (ret != WmErrorCode::WM_OK) {
        return NapiThrowError(env, ret, "[window][on]msg: register " + cbType + " failed");
    }
...
napi_value JsWindow::OnUnregisterWindowCallback(napi_env env, napi_callback_info info)
{
    if (windowToken_ == nullptr) {
        WLOGFE("Window is nullptr");
        return NapiThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY, "[window][off]msg: invalid window");
    }
    size_t argc = 4;
    napi_value argv[4] = {nullptr};
    napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr);
    ...
    if (ret != WmErrorCode::WM_OK) {
        return NapiThrowError(env, ret, "[window][off]msg: unregister " + cbType + " failed");
    }
