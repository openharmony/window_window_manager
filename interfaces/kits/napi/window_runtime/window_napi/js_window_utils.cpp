...
{
    if (jsObject == nullptr) {
        TLOGE(WmsLogTag::WMS_PC, "Failed to convert parameter to window mask with alpha, jsObject is nullptr");
        return false;
    }
    bool isTypedArray = false;
    napi_is_typedarray(env, jsObject, &isTypedArray);
    if (!isTypedArray) {
        TLOGE(WmsLogTag::WMS_PC, "Failed to convert parameter to window mask with alpha, not typed array");
        return false;
    }
    ...
    if (type != napi_uint8_array) {
        TLOGE(WmsLogTag::WMS_PC, "Failed to convert parameter to window mask with alpha, not uint8 array");
        return false;
    }
    ...
    WindowMaskWithAlphaParams& params, const sptr<Window>& windowToken)
{
    ...
    if (argc < minRequiredParams) {
        TLOGE(WmsLogTag::WMS_PC, "Argc is invalid: %{public}zu, required: %{public}zu", argc, minRequiredParams);
        return WmErrorCode::WM_ERROR_INVALID_PARAM;
    }
    if (!GetWindowMaskWithAlphaFromJsValue(env, argv[INDEX_ZERO], &params.maskData, params.byteLength)) {
        TLOGE(WmsLogTag::WMS_PC, "GetWindowMaskWithAlphaFromJsValue failed");
        return WmErrorCode::WM_ERROR_INVALID_PARAM;
    }
    if (!ConvertFromJsValue(env, argv[INDEX_ONE], params.maskWidth)) {
        TLOGE(WmsLogTag::WMS_PC, "Get maskWidth failed");
        return WmErrorCode::WM_ERROR_INVALID_PARAM;
    }
    if (!ConvertFromJsValue(env, argv[INDEX_TWO], params.maskHeight)) {
        TLOGE(WmsLogTag::WMS_PC, "Get maskHeight failed");
        return WmErrorCode::WM_ERROR_INVALID_PARAM;
    }
    ...
    if (expectedSize == 0 || params.byteLength != expectedSize) {
        TLOGE(WmsLogTag::WMS_PC, "windowMask size %{public}zu not equal to %{public}zu",
