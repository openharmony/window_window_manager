...
napi_value JsWindowStage::OnSetImageForRecent(napi_env env, napi_callback_info info)
{
    ...
    if(pixelMap != nullptr){
        ...
        TLOGI(WmsLogTag::WMS_PATTERN, "[NAPI][SetImageForRecent] parse ok type=resId imageFit=%{public}d id=%{public}d",
    ...
    auto asyncTask = [weakWindow = windowScene_, where, env, imgResourceId, pixelMap, imageFit, task = napiAsyncTask] {
        ...
        if (ret == WmErrorCode::WM_OK) {
            ...
            TLOGE(WmsLogTag::WMS_PATTERN, "%{public}s set image for recent failed, ret=%{public}d,type=%{public}d", where, ret, pixelMap ? "pixelMap" : "resId");
