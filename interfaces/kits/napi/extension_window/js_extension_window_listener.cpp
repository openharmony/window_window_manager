    ...
    const std::shared_ptr<RSTransaction>& rsTransaction)
{
    ...
    if (reason == WindowSizeChangeReason::ROTATION) {
        jsCallback();
    } else if (napi_status::napi_ok != napi_send_event(env_, jsCallback, napi_eprio_high, "OnSizeChange")) {
...
void JsExtensionWindowListener::OnRectChange(Rect rect, WindowSizeChangeReason reason)
{
    ...
    if (napi_status::napi_ok != napi_send_event(env_, jsCallback, napi_eprio_high, "OnRectChange")) {
        TLOGE(WmsLogTag::WMS_UIEXT, "send event failed");
    }
    currRect_ = rect;
}
    ...
    const sptr<OccupiedAreaChangeInfo>& info)
{
    ...
    if (napi_status::napi_ok != napi_send_event(env_, jsCallback, napi_eprio_high, "OnAvoidAreaChanged")) {
        TLOGE(WmsLogTag::WMS_UIEXT, "send event failed");
    }
}
                                             ...
                                             const std::shared_ptr<RSTransaction>& rsTransaction)
{
    ...
    if (napi_status::napi_ok != napi_send_event(env_, jsCallback, napi_eprio_high, "OnSizeChange")) {
        TLOGE(WmsLogTag::WMS_UIEXT, "send event failed");
    }
}
