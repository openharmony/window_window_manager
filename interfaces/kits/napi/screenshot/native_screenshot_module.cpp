...
{
    GNAPI_LOG("Get Screenshot Option: isFullScreenCapture");
    ...
    if (isFullScreenCapture != nullptr && GetType(env, isFullScreenCapture) == napi_boolean) {
        ...
        GNAPI_LOG("isFullScreenCapture: %{public}d", param->option.isFullScreenCapture);
    } else {
        GNAPI_LOG("isFullScreenCapture failed, invalid param, use default false.");
    }
}
