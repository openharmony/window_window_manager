...
void AniWindow::SetTitleAndDockHoverShown(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean isTitleHoverShown, ani_boolean isDockHoverShown)
{
...
void AniWindow::OnSetTitleAndDockHoverShown(ani_env* env, ani_boolean isTitleHoverShown, ani_boolean isDockHoverShown)
{
    TLOGI(WmsLogTag::WMS_LAYOUT_PC, "[ANI]");
    auto window = GetWindow();
    if (window == nullptr) {
    ...
    TLOGI(WmsLogTag::WMS_LAYOUT_PC, "window [%{public}u, %{public}s] [%{public}d, %{public}d]",
        window->GetWindowId(), window->GetWindowName().c_str(), isTitleHoverShown, isDockHoverShown);
}
...
void AniWindow::SetWindowDelayRaiseOnDrag(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean isDelayRaise)
{
    TLOGI(WmsLogTag::DEFAULT, "[ANI]");
    AniWindow* aniWindow = reinterpret_cast<AniWindow*>(nativeObj);
    if (aniWindow != nullptr) {
        aniWindow->OnSetWindowDelayRaiseOnDrag(env, isDelayRaise);
    } else {
        TLOGE(WmsLogTag::DEFAULT, "[ANI] aniWindow is nullptr");
    }
}

void AniWindow::OnSetWindowDelayRaiseOnDrag(ani_env* env, ani_boolean isDelayRaise)
{
    TLOGI(WmsLogTag::DEFAULT, "[ANI]");
    auto window = GetWindow();
    if (window == nullptr) {
    ...
    WmErrorCode ret = WM_JS_TO_ERROR_CODE_MAP.at(window->SetWindowDelayRaiseEnabled(isDelayRaise));
    TLOGI(WmsLogTag::DEFAULT, "Window [%{public}u, %{public}s] show with ret=%{public}d",
        window->GetWindowId(), window->GetWindowName().c_str(), ret);
    if (ret != WmErrorCode::WM_OK) {
        AniWindowUtils::AniThrowError(env, ret, "[ANI] set title button visible failed");
    }
