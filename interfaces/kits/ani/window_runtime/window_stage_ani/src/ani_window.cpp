...
ani_int AniWindow::GetSubWindowZLevel(ani_env* env, ani_object obj, ani_long nativeObj)
{
    ...
    if (aniWindow == nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.getSubWindowZLevel",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
ani_boolean AniWindow::IsFocused(ani_env* env, ani_object obj, ani_long nativeObj)
{
    ...
    if (aniWindow == nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.isFocused",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            WindowFocusErrorMsgHelper::GetErrorMsg(WindowFocusApiType::IS_FOCUSED,
                WMError::WM_ERROR_NULLPTR));
        return ANI_ERROR;
    }
...
void AniWindow::OnRaiseAboveTarget(ani_env* env, ani_int windowId)
{
    ...
    WMError ret = window->RaiseAboveTarget(windowId);
    if (ret != WMError::WM_OK) {
...
void AniWindow::OnRaiseToAppTop(ani_env* env)
{
    ...
    if (wmError != WMError::WM_OK) {
        WmErrorCode ret = WM_JS_TO_ERROR_CODE_MAP.at(wmError);
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.raiseToAppTop", ret);
        std::string errMsg = WindowFocusErrorMsgHelper::GetErrorMsg(WindowFocusApiType::RAISE_TO_APP_TOP, wmError);
        AniWindowUtils::AniThrowError(env, ret, errMsg);
    }
}
...
void AniWindow::SetTopmost(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean isTopmost)
{
    ...
    if (aniWindow != nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setTopmost",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            WindowFocusErrorMsgHelper::GetErrorMsg(WindowFocusApiType::SET_TOPMOST,
                WMError::WM_ERROR_NULLPTR));
    }
}
...
void AniWindow::OnSetTopmost(ani_env* env, ani_boolean isTopmost)
{
    ...
    if (!Permission::IsSystemCalling()) {
        ...
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_NOT_SYSTEM_APP,
            "[window][setTopmost]msg:setTopmost permission denied");
        return;
    }
    auto window = GetWindow();
    if (window == nullptr) {
    ...
    if (ret != WmErrorCode::WM_OK) {
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setTopmost", ret);
...
void AniWindow::OnRaiseMainWindowAboveTarget(ani_env* env, ani_int windowId)
{
    ...
    if (!Permission::IsSystemCallingOrStartByHdcd(true)) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.raiseMainWindowAboveTarget",
            WmErrorCode::WM_ERROR_NOT_SYSTEM_APP);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_NOT_SYSTEM_APP,
            "[window][raiseMainWindowAboveTarget]msg: permission denied, require system application");
        return;
    }
    WMError ret = window->RaiseMainWindowAboveTarget(windowId);
    if (ret != WMError::WM_OK) {
        WmErrorCode wmErrorCode = WM_JS_TO_ERROR_CODE_MAP.at(ret);
...
void AniWindow::SetWindowFocusable(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean isFocusable)
{
    ...
    if (aniWindow != nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowFocusable",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            WindowFocusErrorMsgHelper::GetErrorMsg(WindowFocusApiType::SET_FOCUSABLE,
                WMError::WM_ERROR_NULLPTR));
    }
}
...
void AniWindow::OnSetWindowFocusable(ani_env* env, ani_boolean isFocusable)
{
    ...
    WMError wmError = window->SetFocusable(isFocusable);
    if (wmError != WMError::WM_OK) {
        WmErrorCode ret = WM_JS_TO_ERROR_CODE_MAP.at(wmError);
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowFocusable", ret);
        std::string errMsg = WindowFocusErrorMsgHelper::GetErrorMsg(WindowFocusApiType::SET_WINDOW_FOCUSABLE, wmError);
        AniWindowUtils::AniThrowError(env, ret, errMsg);
    }
}
...
void AniWindow::RequestFocus(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean isFocused)
{
    ...
    if (aniWindow != nullptr) {
        ...
        TLOGE(WmsLogTag::WMS_HIERARCHY, "[ANI] aniWindow is nullptr");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.requestFocus",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
void AniWindow::OnRequestFocus(ani_env* env, ani_boolean isFocused)
{
    ...
    if (!Permission::IsSystemCalling()) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.requestFocus",
            WmErrorCode::WM_ERROR_NOT_SYSTEM_APP);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_NOT_SYSTEM_APP,
            "[window][requestFocus]msg: RequestFocus permission denied!");
        return;
    }
    auto window = GetWindow();
    if (window == nullptr) {
    ...
    if (ret != WmErrorCode::WM_OK) {
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.requestFocus", ret);
...
void AniWindow::OnSetSubWindowModal(ani_env* env, ani_boolean isModal)
{
    ...
    WMError wmError = window->SetSubWindowModal(isModal);
    if (wmError != WMError::WM_OK) {
        WmErrorCode ret = WM_JS_TO_ERROR_CODE_MAP.at(wmError);
...
void AniWindow::SetWindowMaskWithAlpha(ani_env* env, ani_object obj, ani_long nativeObj,
    ani_object windowMask, ani_int maskWidth, ani_int maskHeight)
{
    TLOGD(WmsLogTag::WMS_EVENT, "[ANI]");
    AniWindow* aniWindow = reinterpret_cast<AniWindow*>(nativeObj);
    if (aniWindow != nullptr) {
        aniWindow->OnSetWindowMaskWithAlpha(env, windowMask, maskWidth, maskHeight);
    } else {
        TLOGE(WmsLogTag::WMS_EVENT, "[ANI]aniWindow is nullptr!");
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
    }
    ...
    ani_int maskHeight)
{
    ...
    if (!WindowHelper::IsSubWindow(window->GetType()) && !WindowHelper::IsAppFloatingWindow(window->GetType())) {
        TLOGE(WmsLogTag::WMS_EVENT, "[ANI]Invalid window type: %{public}u", window->GetType());
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_INVALID_CALLING, "Invalid window type");
        return;
    }
    ...
    sptr<Window>& window, WindowMaskWithAlphaParseParams& params)
{
    if (params.rawMaskWidth <= 0 || params.rawMaskHeight <= 0) {
        TLOGE(WmsLogTag::WMS_EVENT, "[ANI]Invalid maskWidth %{public}d or maskHeight %{public}d",
    ...
    if (expectedSize == 0 || static_cast<size_t>(params.byteLength) != expectedSize) {
        TLOGE(WmsLogTag::WMS_EVENT, "[ANI]windowMask length %{public}zu not equal to %{public}zu",
...
void AniWindow::OnSetWindowTitle(ani_env* env, ani_string titleName)
{
    ...
    if (errorCode != WmErrorCode::WM_OK) {
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowTitle.error", errorCode);
...
void AniWindow::OnSetHandwritingFlag(ani_env* env, ani_boolean enable)
{
    ...
    if (errCode != WmErrorCode::WM_OK) {
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setHandwritingFlag.error", errCode);
...
ani_boolean AniWindow::IsWindowHighlighted(ani_env* env, ani_object obj, ani_long nativeObj)
{
    ...
    if (aniWindow != nullptr) {
        ...
        TLOGE(WmsLogTag::WMS_FOCUS, "[ANI] aniWindow is nullptr");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.isWindowHighlighted",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
...
void AniWindow::OnKeepKeyboardOnFocus(ani_env* env, ani_boolean keepKeyboardFlag)
{
    if (windowToken_ == nullptr) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.keepKeyboardOnFocus",
            WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            "[window][keepKeyboardOnFocus]msg:WindowToken_ is nullptr");
        return;
    }
    if (!WindowHelper::IsSystemWindow(windowToken_->GetType()) &&
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.keepKeyboardOnFocus",
            WmErrorCode::WM_ERROR_INVALID_CALLING);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_INVALID_CALLING,
            "[window][keepKeyboardOnFocus]msg:not system window or app subwindow");
        return;
    }
...
ani_object AniWindow::SetRaiseByClickEnabled(ani_env* env, ani_boolean enable)
{
    ...
    if (ret != WMError::WM_OK) {
        ...
        return AniWindowUtils::AniThrowError(env, wmErrorCode,
...
ani_boolean AniWindow::IsFloatNavigationAvoidAreaEnabled(ani_env* env)
{
    if (windowToken_ == nullptr) {
        TLOGE(WmsLogTag::WMS_IMMS, "windowToken_ is null");
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
...
void AniWindow::SetFloatNavigationAvoidAreaEnabled(ani_env* env, ani_boolean enabled)
{
    if (windowToken_ == nullptr) {
        TLOGE(WmsLogTag::WMS_IMMS, "windowToken_ is null");
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY);
...
void AniWindow::SetFollowParentWindowLayoutEnabled(ani_env* env, ani_boolean enable)
{
    ...
    TLOGD(WmsLogTag::WMS_LAYOUT, "[ANI] success, windowId: %{public}u, enable: %{public}u", windowId, isFollow);
}
    ...
    std::function<WMError(sptr<Window>&, const Position&, Position&)> transformFunc)
{
    if (windowToken_ == nullptr || transformFunc == nullptr) {
        ...
        return AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            "[window][handlePositionTransform]msg: window or transformFunc is nullptr");
    }
...
void AniWindow::SetRotationLocked(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean locked)
{
    ...
    if (!aniWindow) {
        TLOGE(WmsLogTag::WMS_ROTATION, "[ANI] aniWindow is nullptr");
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            "[window][setRotationLocked]msg: The window is not created or destroyed.");
        return;
    }
...
ani_boolean AniWindow::GetRotationLocked(ani_env* env, ani_object obj, ani_long nativeObj)
{
    ...
    TLOGE(WmsLogTag::WMS_ROTATION, "[ANI] aniWindow is nullptr");
    AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
        "[window][getRotationLocked]msg: The window is not created or destroyed.");
    return ANI_FALSE;
}
...
void AniWindow::OnSetWindowDelayRaiseOnDrag(ani_env* env, ani_boolean isEnabled)
{
    ...
    if (ret != WMError::WM_OK) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.setWindowDelayRaiseOnDrag", errorCode);
        AniWindowUtils::AniThrowError(env, errorCode,
            WindowFocusErrorMsgHelper::GetErrorMsg(WindowFocusApiType::SET_WINDOW_DELAY_RAISE_ENABLED, ret));
        return;
    }
}
    ...
    ani_object attachOptions)
{
    ...
    aniWindow->OnAttachToParentWindow(env, anchorInfo, attachOptions);
}
    ...
    std::unique_ptr<AniWindowRegisterManager>& registerManager, WindowAnchorInfo::AttachOptions& options)
{
    ...
    if (!isUndefinedCurrentLayoutMode) {
        std::string currentLayoutMode = AniWindowUtils::ANIStringToStdString(env,
    ...
    ani_object attachOptions)
{
    if (!Permission::IsSystemCalling()) {
        ...
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.attachLayoutToParentWindow",
            WmErrorCode::WM_ERROR_NOT_SYSTEM_APP);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_NOT_SYSTEM_APP, "not system application");
        return;
    }
    if (windowToken_ == nullptr) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "[ANI] window is nullptr");
    ...
    if (!WindowHelper::IsSubWindow(windowToken_->GetType())) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "[ANI] only sub window is valid");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.attachLayoutToParentWindow",
            WmErrorCode::WM_ERROR_INVALID_CALLING);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_INVALID_CALLING);
        return;
    }
    WindowAnchorInfo acceptWindowAnchorInfo = {true, true, WindowAnchor::TOP_START, 0, 0};
    ani_boolean isUndefinedAnchorInfo;
    ani_status aniAnchorRet = env->Reference_IsUndefined(anchorInfo, &isUndefinedAnchorInfo);
    if (aniAnchorRet != ANI_OK) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "[ANI] Check enum_object isUndefined fail");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.attachLayoutToParentWindow",
            WmErrorCode::WM_ERROR_INVALID_PARAM);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM);
    ...
    if (!isUndefinedAnchorInfo && !AniWindowUtils::ParseWindowAnchorInfo(env, anchorInfo, acceptWindowAnchorInfo)) {
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.attachLayoutToParentWindow",
            WmErrorCode::WM_ERROR_INVALID_PARAM);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM, "Failed to parse windowAnchorInfo");
        return;
    }
    ani_boolean isUndefinedOptions;
    ani_status aniOptionRet = env->Reference_IsUndefined(attachOptions, &isUndefinedOptions);
    if (aniOptionRet != ANI_OK) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "[ANI] Check enum_object isUndefined fail");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.attachLayoutToParentWindow",
            WmErrorCode::WM_ERROR_INVALID_PARAM);
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_INVALID_PARAM);
    ...
    if (!isUndefinedOptions) {
        ParseAttachOptions(windowToken_, env, attachOptions, registerManager_, acceptWindowAnchorInfo.attachOptions);
    }
    ...
    if (errorCode != WmErrorCode::WM_OK) {
        TLOGE(WmsLogTag::WMS_SUB, "[ANI] failed, errorCode:%{public}d ", errorCode);
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.attachLayoutToParentWindow", errorCode);
        AniWindowUtils::AniThrowError(env, errorCode, "[ANI] attach to parent window failed.");
        return;
    }
}

void AniWindow::DetachLayoutToParentWindow(ani_env* env, ani_object obj, ani_long nativeObj)
{
    TLOGD(WmsLogTag::WMS_LAYOUT, "[ANI]");
    AniWindow* aniWindow = reinterpret_cast<AniWindow*>(nativeObj);
    if (!aniWindow) {
    ...
    aniWindow->OnDetachLayoutToParentWindow(env);
}

void AniWindow::OnDetachLayoutToParentWindow(ani_env* env)
{
    ...
    if (sizeChangeRet != WmErrorCode::WM_OK) {
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.detachLayoutToParentWindow",
            sizeChangeRet);
        AniWindowUtils::AniThrowError(env, sizeChangeRet);
        return;
    }
    ...
    WmErrorCode errorCode = (it != WM_JS_TO_ERROR_CODE_MAP.end()) ? it->second : WmErrorCode::WM_ERROR_STATE_ABNORMALLY;
    if (errorCode != WmErrorCode::WM_OK) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "[ANI] failed");
        HISTOGRAM_ENUMERATION_ERROR_CODE("ArkUI.window.detachLayoutToParentWindow", errorCode);
        AniWindowUtils::AniThrowError(env, errorCode, "[ANI] detach from parent window failed.");
        return;
    }
}
...
void AniWindow::RestoreMainWindow(ani_env* env, ani_object obj, ani_long nativeObj, ani_object wantParameters)
{
    ...
    aniWindow->OnRestoreMainWindow(env, wantParameters);
}
    ...
    ani_long nativeObj, ani_boolean enable)
{
    ...
    if (aniWindow == nullptr || aniWindow->GetWindow() == nullptr) {
        TLOGE(WmsLogTag::WMS_HIERARCHY, "[ANI] windowToken_ is nullptr");
        AniWindowUtils::AniThrowError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY,
            "[window][setRaiseByClickEnabled]msg: windowToken_ is nullptr");
        return ANI_ERROR;
    }
    aniWindow->SetRaiseByClickEnabled(env, enable);
    TLOGI(WmsLogTag::WMS_HIERARCHY, "[ANI] setRaiseByClickEnabled end");
    return ANI_OK;
}
...
static ani_boolean IsFloatNavigationAvoidAreaEnabled(ani_env* env, ani_object obj, ani_long nativeObj)
{
    using namespace OHOS::Rosen;
    TLOGI(WmsLogTag::WMS_IMMS, "[ANI]");
    AniWindow* aniWindow = reinterpret_cast<AniWindow*>(nativeObj);
    ...
    return aniWindow->IsFloatNavigationAvoidAreaEnabled(env);
}

static void SetFloatNavigationAvoidAreaEnabled(ani_env* env, ani_object obj, ani_long nativeObj, ani_boolean enabled)
{
    using namespace OHOS::Rosen;
    TLOGI(WmsLogTag::WMS_IMMS, "[ANI]");
    AniWindow* aniWindow = reinterpret_cast<AniWindow*>(nativeObj);
    ...
    aniWindow->SetFloatNavigationAvoidAreaEnabled(env, enabled);
}
...
ani_status OHOS::Rosen::ANI_Window_Constructor(ani_vm *vm, uint32_t *result)
{
    ...
    std::array methods = {
        ...
        ani_native_function {"attachLayoutToParentWindow",
            ...
            reinterpret_cast<void *>(AniWindow::AttachLayoutToParentWindow)},
        ani_native_function {"detachLayoutToParentWindow", "l:",
            reinterpret_cast<void *>(AniWindow::DetachLayoutToParentWindow)},
        ...
        ani_native_function {"isFloatNavigationAvoidAreaEnabled", "l:z",
            reinterpret_cast<void *>(IsFloatNavigationAvoidAreaEnabled)},
        ani_native_function {"setFloatNavigationAvoidAreaEnabled", "lz:",
            reinterpret_cast<void *>(SetFloatNavigationAvoidAreaEnabled)},
