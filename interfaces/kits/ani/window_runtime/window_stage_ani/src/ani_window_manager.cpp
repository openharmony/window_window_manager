...
ani_object AniWindowManager::GetAllWindowLayoutInfo(ani_env* env, ani_long nativeObj, ani_int displayId)
{
...
ani_object AniWindowManager::OnGetAllWindowLayoutInfo(ani_env* env, ani_int displayId)
{
    TLOGI(WmsLogTag::WMS_ATTRIBUTE, "[ANI]");
...
ani_object AniWindowManager::OnGetSnapshot(ani_env* env, ani_int windowId)
{
    ...
    auto nativePixelMap = Media::PixelMapAniTaiheAni::CreateEtsPixelMap(env, pixelMap);
...
ani_object AniWindowManager::OnGetVisibleWindowInfo(ani_env* env)
{
    ...
    if (apiVersion < API_VERSION_18 && !Permission::IsSystemCalling()) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "permission denied! api%{public}u", apaiVersion);
