...
void AniWindowStage::SetWindowRectAutoSave(ani_env* env, ani_class cls, ani_long nativeObj, ani_boolean enabled)
{
    TLOGI(WmsLogTag::DEFAULT, "[ANI]");
    AniWindowStage* aniWindowStage = reinterpret_cast<AniWindowStage*>(nativeObj);
    if (aniWindowStage != nullptr) {
        aniWindowStage->OnSetWindowRectAutoSave(env, enabled);
    } else {
        TLOGE(WmsLogTag::DEFAULT, "[ANI] aniWindowStage is nullptr");
    }
}

void AniWindowStage::OnSetWindowRectAutoSave(ani_env* env, ani_boolean enabled)
{
    auto windowScene = GetWindowScene().lock();
    if (windowScene == nullptr) {
    ...
    WmErrorCode ret = WM_JS_TO_ERROR_CODE_MAP.at(mainWindow->SetWindowRectAutoSave(enabled));
    if (ret != WmErrorCode::WM_OK) {
        AniWindowUtils::AniThrowError(env, ret);
        return;
    }
