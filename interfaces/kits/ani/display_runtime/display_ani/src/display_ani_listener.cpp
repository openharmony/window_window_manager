...
void DisplayAniListener::OnAttributeChange(DisplayId displayId, const std::vector<std::string>& attributes)
{
    ...
    auto task = [vm = vm_, attributes, displayId, weakRef = weakRef_] {
        auto thisListener = weakRef.promote();
        if (thisListener) {
            auto aniVm = AniVm(vm);
            auto env = aniVm.GetAniEnv();
            if (env == nullptr) {
            ...
            thisListener->ProcessAttributeCallbacks(env, attributes, displayId);
        }
    };
    if (!eventHandler_) {
        TLOGE(WmsLogTag::DMS, "[ANI] get main event handler failed!");
...
void DisplayAniListener::OnFoldAngleChanged(std::vector<float> foldAngles)
{
    ...
    if (vm_ != nullptr) {
        ...
        for (auto oneAniCallback : it->second) {
            auto task = [self = weakRef_, vm = vm_, oneAniCallback, foldAngles] () {
                auto aniVm = AniVm(vm);
                auto env = aniVm.GetAniEnv();
                if (env == nullptr) {
                    TLOGE(WmsLogTag::DMS, "[ANI] env got from vm is nullptr");
...
void DisplayAniListener::OnCaptureStatusChanged(bool isCapture)
{
    ...
    if (vm_ != nullptr) {
        ...
        for (auto oneAniCallback : it->second) {
            auto task = [self = weakRef_, vm = vm_, oneAniCallback, isCapture] () {
                auto aniVm = AniVm(vm);
                auto env = aniVm.GetAniEnv();
                if (env == nullptr) {
                    TLOGE(WmsLogTag::DMS, "[ANI] env got from vm is nullptr");
...
void DisplayAniListener::OnDisplayModeChanged(FoldDisplayMode foldDisplayMode)
{
    ...
    if (vm_ != nullptr) {
        ...
        for (auto oneAniCallback : it->second) {
            auto task = [self = weakRef_, vm = vm_, oneAniCallback, foldDisplayMode] () {
                auto aniVm = AniVm(vm);
                auto env = aniVm.GetAniEnv();
                if (env == nullptr) {
                    TLOGE(WmsLogTag::DMS, "[ANI] env got from vm is nullptr");
...
void DisplayAniListener::OnBrightnessInfoChanged(DisplayId id, const ScreenBrightnessInfo& brightnessInfo)
{
    ...
    std::lock_guard<std::mutex> lock(aniCallbackMtx_);
    if (aniCallback_.empty()) {
        TLOGE(WmsLogTag::DMS, "[ANI] brightnessInfoChang not register!");
        return;
    }
