...
ani_status CreateBusinessError(ani_env* env, int32_t error, std::string message, ani_object* err)
{
    ...
    status = env->Object_New(aniClass, aniCtor, err, aniMsg, static_cast<ani_object>(AniGetUndefined(env)));
    if (status != ANI_OK) {
...
ani_object CreateAniFvWindowInfoObject(ani_env* env, const sptr<Window>& window, const FloatViewWindowInfo &windowInfo,
...
ani_object CreateAniFloatViewLimitsObject(ani_env* env, const FloatViewLimits& limits)
{
    ...
    ret = env->Class_FindMethod(limitsClass, "<ctor>",
        "C{@ohos.window.window.Size}C{@ohos.window.window.Size}C{escompat.Array}:", &ctor);
    if (ret != ANI_OK) {
        TLOGE(WmsLogTag::WMS_SYSTEM, "[ANI]find constructor failed");
        return static_cast<ani_object>(AniGetUndefined(env));
    }
    ...
    if (env->Array_New(limits.ratioLimits_.size(),
        static_cast<ani_object>(AniGetUndefined(env)), &ratioLimits) != ANI_OK) {
