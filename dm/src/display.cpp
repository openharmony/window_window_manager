...
DisplayId Display::GetId() const
{
    if (pImpl_ == nullptr || pImpl_->GetDisplayInfo() == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "pImpl_ or pImpl_->GetDisplayInfo is nullptr");
        return DisplayId(0);
    }
...
void Display::UpdateDisplayInfo(sptr<DisplayInfo> displayInfo) const
{
    if (displayInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "displayInfo is invalid");
        return;
    }
    if (pImpl_ == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "pImpl_ is nullptr");
        return;
    }
