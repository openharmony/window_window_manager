...
public:
    ...
    void OnDisplayModeChanged(FoldDisplayMode displayMode)
    {
        if (innerDisplayModeChangeFunc_ == NULL) {
            TLOGI(WmsLogTag::DMS_DM, "[DMNDK] callback is null");
            return;
        }
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] displayMode callback displayMode=%{public}d", displayMode);
...
public:
    ...
    {
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] current not support create callback.");
    }
    void OnDestroy(DisplayId)
    {
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] current not support delete callback.");
    }
    void OnChange(DisplayId displayId)
    {
        ...
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] callback displayId=%{public}" PRIu64, displayId);
        ...
        if (display != nullptr) {
            TLOGI(WmsLogTag::DMS_DM, "[DMNDK] callback rotation=%{public}d orientation=%{public}d",
...
static sptr<DisplayInfo> OH_GetDefaultDisplayInfo()
{
    ...
    if (defaultDisplay == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get default display null.");
        return nullptr;
    }
    auto info = defaultDisplay->GetDisplayInfo();
    if (info == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get default display info null.");
        return nullptr;
    }
    return info;
}
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayId(uint64_t *displayId)
{
    if (displayId == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input displayId null.");
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayWidth(int32_t *displayWidth)
{
    if (displayWidth == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input displayWidth null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayHeight(int32_t *displayHeight)
{
    if (displayHeight == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input displayHeight null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
    ...
    NativeDisplayManager_Rotation *displayRotation)
{
    if (displayRotation == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input displayRotation null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
    ...
    NativeDisplayManager_Orientation *displayOrientation)
{
    if (displayOrientation == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input displayOrientation null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayVirtualPixelRatio(float *virtualPixel)
{
    if (virtualPixel == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input virtualPixel null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayRefreshRate(uint32_t *refreshRate)
{
    if (refreshRate == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input refreshRate null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayDensityDpi(int32_t *densityDpi)
{
    if (densityDpi == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input densityDpi null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayDensityPixels(float *densityPixels)
{
    if (densityPixels == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input densityPixels null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
NativeDisplayManager_ErrorCode OH_NativeDisplayManager_GetDefaultDisplayScaledDensity(float *scaledDensity)
{
    if (scaledDensity == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input scaledDensity null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
{
    if (xDpi == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input xDpi null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
{
    if (yDpi == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input yDpi null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    auto displayInfo = OH_GetDefaultDisplayInfo();
    if (displayInfo == nullptr) {
...
static NativeDisplayManager_Rect* OH_CreateBoundingRects(const std::vector<DMRect> &boundingRects)
{
    ...
    if (ohBoundingRects == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memory failed.");
        return NULL;
    }
    ...
    if (retMemset != 0) {
        ...
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset failed.");
        return NULL;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] bounding size:%{public}d.", boundSize);
    ...
    NativeDisplayManager_CutoutInfo *ohCutoutInfo)
{
    if (ohCutoutInfo == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] ohCutoutInfo is null.");
        return;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] set waterfall Area.");
...
static NativeDisplayManager_CutoutInfo* OH_CreateCutoutInfoObject(const sptr<CutoutInfo> &cutoutInfo)
{
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] boundingRectsLength=%{public}d.", ohCutoutInfo->boundingRectsLength);
    ohCutoutInfo->boundingRects = OH_CreateBoundingRects(boundingRects);
    if (ohCutoutInfo->boundingRects == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] create bounding rects failed.");
        free(ohCutoutInfo);
        ohCutoutInfo = NULL;
        return NULL;
    }
...
{
    if (cutoutInfo == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input cutoutInfo null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get display cutout info.");
    sptr<Display> defaultDisplay = DisplayManager::GetInstance().GetDefaultDisplaySync();
    if (defaultDisplay == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get cutout info (display) null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    sptr<CutoutInfo> cutoutInfoInner = defaultDisplay->GetCutoutInfo();
    if (cutoutInfoInner == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get cutout info (from display) null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    *cutoutInfo = OH_CreateCutoutInfoObject(cutoutInfoInner);
    if (*cutoutInfo == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] convert cutout info null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get display cutout info success.");
    return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
}
...
{
    if (cutoutInfo == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input cutoutInfo null pointer.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    ...
    free(cutoutInfo);
    cutoutInfo = NULL;
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] destroy cutoutInfo end.");
    return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
}
...
bool OH_NativeDisplayManager_IsFoldable()
{
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get display isFoldable=%{public}d.", isFoldable);
    return isFoldable;
}
...
{
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get fold display mode.");
    if (foldDisplayMode == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input foldDisplayMode null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    if (!DisplayManager::GetInstance().IsFoldable()) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] device is not foldable.");
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] current fold display mode: %{public}d.", *foldDisplayMode);
    return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
}
...
{
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] register fold display mode change listener.");
    if (displayModeChangeCallback == NULL || listenerIndex == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input params null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    if (!DisplayManager::GetInstance().IsFoldable()) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] device is not foldable.");
    ...
    if (CheckFoldChangeHasRegistered(displayModeChangeCallback)) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] input params error (has registered).");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    ...
    if (displayModeListener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] display mode listener MakeSptr fail.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    ...
    if (ret != DMError::DM_OK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] display mode listener register failed ret=%{public}d.", ret);
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    *listenerIndex = registerCount++;
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] register fold change success and listenerIndex= %{public}d.", *listenerIndex);
    return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
}

NativeDisplayManager_ErrorCode OH_NativeDisplayManager_UnregisterFoldDisplayModeChangeListener(uint32_t listenerIndex)
{
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] unregister fold display mode change listener %{public}d.", listenerIndex);
    if (!DisplayManager::GetInstance().IsFoldable()) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] unregister fail(device is not foldable).");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_DEVICE_NOT_SUPPORTED;
    }
    std::unique_lock<std::shared_mutex> lock(foldChangeMutex);
    auto iter = g_foldDisplayModeChangeListenerMap.find(listenerIndex);
    if (iter == g_foldDisplayModeChangeListenerMap.end()) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] unregister listener fail(not find register info).");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    ...
    if (iter->second != nullptr) {
        ...
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] unregister listener ert=%{public}d", ret);
    }
...
{
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] register display change listener.");
    if (displayChangeCallback == NULL || listenerIndex == NULL) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] register fail(input params null).");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    ...
    if (displayListener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] register display change MakeSptr fail.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    static std::atomic<uint32_t> registerCount = 1;
    DMError ret = DisplayManager::GetInstance().RegisterDisplayListener(displayListener);
    if (ret != DMError::DM_OK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] register failed ret=%{public}d.", ret);
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    *listenerIndex = registerCount++;
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] register listenerIndex= %{public}d.", *listenerIndex);
    return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
}

NativeDisplayManager_ErrorCode OH_NativeDisplayManager_UnregisterDisplayChangeListener(uint32_t listenerIndex)
{
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] unregister display change listener %{public}d.", listenerIndex);
    std::unique_lock<std::shared_mutex> lock(displayChangeMutex);
    auto iter = g_displayChangeListenerMap.find(listenerIndex);
    if (iter == g_displayChangeListenerMap.end()) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] unregister fail(not find register info).");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    DMError ret = DMError::DM_OK;
    if (iter->second != nullptr) {
        ...
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] unregister listener ret=%{public}d", ret);
    }
    return ret == DMError::DM_OK ? NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK :
        NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
}
...
static void NativeDisplayManager_SetColorSpace(NativeDisplayManager_DisplayInfo *displayInfo, sptr<DisplayInfo> info)
{
    ...
    if (colorSpaces.empty()) {
        TLOGW(WmsLogTag::DMS_DM, "[DMNDK] colorSpaces is empty displayId=%{public}u", displayInfo->id);
        return;
    }
    ...
    if (displayInfo->colorSpace == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] malloc color space failed");
        return;
    }
    auto retMemset = memset_s(displayInfo->colorSpace, sizeof(NativeDisplayManager_DisplayColorSpace), 0,
        sizeof(NativeDisplayManager_DisplayColorSpace));
    if (retMemset != EOK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset color space failed");
    ...
    if (displayInfo->colorSpace->colorSpaces == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] malloc color spaces failed");
        DISPLAY_MANAGER_FREE_MEMORY(displayInfo->colorSpace);
        return;
    }
    ...
    if (retMemset != EOK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset color spaces failed");
    ...
    for (const auto colorSpace : colorSpaces) {
        ...
        if (DM_NATIVE_TO_NDK_COLOR_SPACE_TYPE_MAP.find(colorSpaceValue) ==
            ...
            TLOGW(WmsLogTag::DMS_DM, "[DMNDK] color spaces[%{public}d] not in map.", colorSpace);
            continue;
        }
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] color spaces count:%{public}u.", colorLoop);
}
...
static void NativeDisplayManager_SetHdrFormat(NativeDisplayManager_DisplayInfo *displayInfo, sptr<DisplayInfo> info)
{
    ...
    if (hdrFormats.empty()) {
        TLOGW(WmsLogTag::DMS_DM, "[DMNDK] hdrFormats is empty displayId=%{public}u", displayInfo->id);
        return;
    }
    ...
    if (displayInfo->hdrFormat == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] malloc hdr format failed");
        return;
    }
    auto retMemset = memset_s(displayInfo->hdrFormat, sizeof(NativeDisplayManager_DisplayHdrFormat), 0,
        sizeof(NativeDisplayManager_DisplayHdrFormat));
    if (retMemset != EOK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset hdr format failed");
    ...
    for (const auto hdrFormat : hdrFormats) {
        ...
        if (DM_NATIVE_TO_NDK_HDR_FORMAT_TYPE_MAP.find(hdrFormatValue) == DM_NATIVE_TO_NDK_HDR_FORMAT_TYPE_MAP.end()) {
            TLOGW(WmsLogTag::DMS_DM, "[DMNDK] hdr format[%{public}d] not in map.", hdrFormat);
            continue;
        }
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] hdr format count:%{public}u", hdrLoop);
}
    ...
    sptr<DisplayInfo> info)
{
    ...
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] set display id[%{public}u] finish.", displayInfo->id);
}
    ...
    NativeDisplayManager_DisplayInfo *displaysInfo)
{
    ...
    for (auto& display : displays) {
        if (display == nullptr) {
            TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get display null.");
            continue;
        }
        auto info = display->GetDisplayInfo();
        if (info == nullptr) {
            TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get display id[%{public}" PRIu64"] info null.", display->GetId());
            continue;
        }
        int ret = memcpy_s(displaysInfo[i].name, OH_DISPLAY_NAME_LENGTH, info->GetName().c_str(),
            OH_DISPLAY_NAME_LENGTH);
        if (ret != EOK) {
            TLOGE(WmsLogTag::DMS_DM, "[DMNDK] failed to memcpy name.");
            return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
        }
    ...
    NativeDisplayManager_DisplayInfo *displaysInfo)
{
    if (displaysLength == 0 || displaysInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] param is null.");
        return;
    }
    ...
    NativeDisplayManager_DisplaysInfo **allDisplays)
{
    ...
    if (displays.empty()) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] displays is empty.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    ...
    if (displaysInner == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] malloc displays inner failed.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    ...
    if (retMemset != EOK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset displays failed.");
    ...
    if (displaysInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] malloc displaysInfo failed.");
        DISPLAY_MANAGER_FREE_MEMORY(displaysInner);
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    ...
    if (retMemset != EOK || setRet != NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset or set displaysInfo failed setRet=%{public}d.", setRet);
...
{
    if (allDisplays == nullptr) {
        TLOGI(WmsLogTag::DMS_DM, "[DMNDK] param is null.");
        return;
    }
    ...
    NativeDisplayManager_ErrorCode *errCode)
{
    ...
    if (info == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get display info null.");
    ...
    if (displayInner == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] malloc display info null.");
        *errCode = NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
        return nullptr;
    }
    ...
    if (retMemset != EOK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memset display info null.");
    ...
    if (ret != EOK) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] memcpy display name failed.");
        DISPLAY_MANAGER_FREE_MEMORY(displayInner);
        *errCode = NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
        return nullptr;
    }
    ...
    NativeDisplayManager_DisplayInfo **displayInfo)
{
    ...
    if (display == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get display by id null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get display id[%{public}" PRIu64"] info", display->GetId());
    ...
    NativeDisplayManager_DisplayInfo **displayInfo)
{
    ...
    if (display == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] get primary display is null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get primary display id[%{public}" PRIu64"].", display->GetId());
    NativeDisplayManager_ErrorCode errorCode = NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
    *displayInfo = NativeDisplayManager_FillDisplayInfo(display, &errorCode);
    return errorCode;
}
...
{
    if (displayInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] free display info is null.");
        return;
    }
    if (displayInfo->colorSpace != nullptr) {
        if (displayInfo->colorSpace->colorSpaces != nullptr) {
...
{
    if (pixelMap == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] pixelMap is null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    ...
    if (errCode == DmErrorCode::DM_ERROR_INVALID_PARAM) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] param error.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_INVALID_PARAM;
    }
    if (errCode == DmErrorCode::DM_ERROR_DEVICE_NOT_SUPPORT) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] not support.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_DEVICE_NOT_SUPPORTED;
    }
    if (errCode == DmErrorCode::DM_ERROR_NO_PERMISSION) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] pixelMap no permission.");
    ...
    *pixelMap = new OH_PixelmapNative(captureImage);
    if (*pixelMap == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "[DMNDK] pixelMap convert pixelMapNative null.");
        return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_ERROR_SYSTEM_ABNORMAL;
    }
    TLOGI(WmsLogTag::DMS_DM, "[DMNDK] get screen capture end.");
    return NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK;
}