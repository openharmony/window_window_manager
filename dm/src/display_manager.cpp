...
public:
    ...
    void OnDisplayCreate(sptr<DisplayInfo> displayInfo) override
    {
        if (displayInfo == nullptr || displayInfo->GetDisplayId() == DISPLAY_ID_INVALID) {
            TLOGE(WmsLogTag::DMS_DM, "OnDisplayCreate, displayInfo is invalid.");
            return;
        }
        if (pImpl_ == nullptr) {
            TLOGE(WmsLogTag::DMS_DM, "OnDisplayCreate, impl is nullptr.");
            return;
        }
    ...
    void OnDisplayDestroy(DisplayId displayId) override
    {
        if (displayId == DISPLAY_ID_INVALID) {
            TLOGE(WmsLogTag::DMS_DM, "OnDisplayDestroy, displayId is invalid.");
            return;
        }
        if (pImpl_ == nullptr) {
            TLOGE(WmsLogTag::DMS_DM, "OnDisplayDestroy, impl is nullptr.");
            return;
        }
    ...
    {
        if (displayInfo == nullptr || displayInfo->GetDisplayId() == DISPLAY_ID_INVALID) {
            TLOGE(WmsLogTag::DMS_DM, "OnDisplayChange, displayInfo is invalid.");
            return;
        }
        if (pImpl_ == nullptr) {
            TLOGE(WmsLogTag::DMS_DM, "OnDisplayChange, impl is nullptr.");
            return;
        }
        TLOGD(WmsLogTag::DMS_DM, "OnDisplayChange. display %{public}" PRIu64", event %{public}u",
...
void DisplayManager::Impl::ClearDisplayStateCallback()
{
    ...
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]Clear displaystatecallback enter");
    displayStateCallback_ = nullptr;
    if (displayStateAgent_ != nullptr) {
        TLOGI(WmsLogTag::DMS_DM, "[UL_POWER]UnregisterDisplayManagerAgent enter and displayStateAgent_ is cleared");
...
void DisplayManager::Impl::ClearFoldStatusCallback()
{
    ...
    if (foldStatusListenerAgent_ != nullptr) {
        ...
        TLOGI(WmsLogTag::DMS_DM, "foldStatusListenerAgent_ is nullptr !");
    }
    if (res != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "UnregisterDisplayManagerAgent FOLD_STATUS_CHANGED_LISTENER failed !");
    }
}
...
void DisplayManager::Impl::ClearFoldAngleCallback()
{
    ...
    if (res != DMError::DM_OK) {
        TLOGE(WmsLogTag::DMS_DM, "ClearFoldAngleCallback FOLD_ANGLE_CHANGED_LISTENER failed !");
    } else {
        TLOGI(WmsLogTag::DMS_DM, "ClearFoldAngleCallback foldAngleListenerAgent_!");
    }
}
...
void DisplayManager::Impl::ClearCaptureStatusCallback()
{
    ...
    if (res != DMError::DM_OK) {
        TLOGE(WmsLogTag::DMS_DM, "ClearCaptureStatusCallback FOLD_ANGLE_CHANGED_LISTENER failed !");
    } else {
        TLOGI(WmsLogTag::DMS_DM, "ClearCaptureStatusCallback captureStatusListenerAgent_!");
    }
}
...
void DisplayManager::Impl::ClearDisplayModeCallback()
{
    ...
    if (displayModeListenerAgent_ != nullptr) {
        ...
        TLOGI(WmsLogTag::DMS_DM, "ClearDisplayStateCallback displayModeListenerAgent_ is nullptr !");
    }
    if (res != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "UnregisterDisplayManagerAgent DISPLAY_MODE_CHANGED_LISTENER failed !");
    }
}

void DisplayManager::Impl::Clear()
{
    TLOGI(WmsLogTag::DMS_DM, "Clear listener");
    std::lock_guard<std::recursive_mutex> lock(mutex_);
    DMError res = DMError::DM_OK;
    ...
    if (res != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "UnregisterDisplayManagerAgent DISPLAY_EVENT_LISTENER failed !");
    }
    res = DMError::DM_OK;
    ...
    if (res != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "UnregisterDisplayManagerAgent DISPLAY_POWER_EVENT_LISTENER failed !");
    }
...
sptr<Display> DisplayManager::Impl::GetDefaultDisplaySync()
{
    ...
    while (retryTimes < MAX_RETRY_NUM) {
        ...
        retryTimes++;
        TLOGW(WmsLogTag::DMS_DM, "Current get display info is null, retry %{public}u times", retryTimes);
        std::this_thread::sleep_for(std::chrono::milliseconds(RETRY_WAIT_MS));
    }
    if (retryTimes >= MAX_RETRY_NUM || displayInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "Get display info failed, please check whether the onscreenchange event is triggered");
        return nullptr;
    }
...
{
    TLOGD(WmsLogTag::DMS_DM, "GetDisplayById start, displayId: %{public}" PRIu64" ", displayId);
    auto currentTime = std::chrono::steady_clock::now();
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
    ...
    if (displayInfo == nullptr) {
        TLOGW(WmsLogTag::DMS_DM, "display null id : %{public}" PRIu64" ", displayId);
        return nullptr;
    }
...
{
    TLOGD(WmsLogTag::DMS_DM, "start, displayId: %{public}" PRIu64" ", displayId);
...
{
    if (g_dmIsDestroyed) {
        TLOGI(WmsLogTag::DMS_DM, "DM has been destructed");
        return nullptr;
    }
    std::lock_guard<std::recursive_mutex> lock(mutex_);
...
sptr<Display> DisplayManager::GetDisplayByScreen(ScreenId screenId)
{
    if (screenId == SCREEN_ID_INVALID) {
        TLOGE(WmsLogTag::DMS_DM, "screenId is invalid.");
        return nullptr;
    }
    sptr<Display> display = pImpl_->GetDisplayByScreenId(screenId);
    if (display == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "get display by screenId failed. screen %{public}" PRIu64"", screenId);
    }
    return display;
}
...
sptr<Display> DisplayManager::Impl::GetDisplayByScreenId(ScreenId screenId)
{
    ...
    if (displayInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "get display by screenId: displayInfo is null");
        return nullptr;
    }
    DisplayId displayId = displayInfo->GetDisplayId();
    if (displayId == DISPLAY_ID_INVALID) {
        TLOGE(WmsLogTag::DMS_DM, "get display by screenId: invalid displayInfo");
        return nullptr;
    }
...
{
    if (displayId == DISPLAY_ID_INVALID) {
        TLOGE(WmsLogTag::DMS_DM, "displayId invalid!");
        return nullptr;
    }
...
std::shared_ptr<Media::PixelMap> DisplayManager::GetSnapshotByPicker(Media::Rect &rect, DmErrorCode* errorCode)
{
    ...
    if (!lock.try_lock()) {
        TLOGE(WmsLogTag::DMS_DM, "try_lock failed!");
        return nullptr;
    }
    std::shared_ptr<Media::PixelMap> screenShot =
    ...
    TLOGI(WmsLogTag::DMS_DM, "snapshot area left:%{public}d, top:%{public}d, width:%{public}d, height:%{public}d",
        rect.left, rect.top, rect.width, rect.height);
    // create crop pixel map
    if (rect.width == 0 || rect.height == 0) {
        TLOGE(WmsLogTag::DMS_DM, "width or height is invalid!");
        return nullptr;
    }
    ...
    if (pixelMap == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "Media pixel map create failed");
        return nullptr;
    }
    return pixelMap;
}
    ...
    DmErrorCode* errorCode, bool isUseDma)
{
    ...
    if (!pImpl_->CheckRectValid(snapShotConfig.imageRect_, oriHeight, oriWidth)) {
        TLOGE(WmsLogTag::DMS_DM, "rect invalid! left %{public}d, top %{public}d, w %{public}d, h %{public}d",
    ...
    if (!pImpl_->CheckSizeValid(snapShotConfig.imageSize_, oriHeight, oriWidth)) {
        TLOGE(WmsLogTag::DMS_DM, "size invalid! w %{public}d, h %{public}d", snapShotConfig.imageSize_.width,
    ...
    if (pixelMap == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "Media::PixelMap::Create failed!");
        return nullptr;
    }
    ...
    const Media::Size &size, int rotation, DmErrorCode* errorCode)
{
    ...
    if (!pImpl_->CheckSizeValid(size, oriHeight, oriWidth)) {
        TLOGE(WmsLogTag::DMS_DM, "size invalid! w %{public}d, h %{public}d", rect.width, rect.height);
        return nullptr;
    }
...
void DisplayManager::AddDisplayIdFromAms(DisplayId displayId, const wptr<IRemoteObject>& abilityToken)
{
    if (abilityToken == nullptr || displayId == DISPLAY_ID_INVALID) {
        TLOGE(WmsLogTag::DMS_DM, "abilityToken is nullptr or display id invalid.");
        return;
    }
    ...
    if (iter != displayIdList_.end()) {
        TLOGI(WmsLogTag::DMS_DM, "abilityToken and display[%{public}" PRIu64"] has been added.", displayId);
    } else {
...
void DisplayManager::RemoveDisplayIdFromAms(const wptr<IRemoteObject>& abilityToken)
{
    if (abilityToken == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "abilityToken is nullptr.");
        return;
    }
    std::lock_guard<std::mutex> lock(displayOperateMutex_);
    if (displayIdList_.empty()) {
        TLOGI(WmsLogTag::DMS_DM, "displayIdList_ is empty.");
        return;
    }
    auto iter = std::find_if(displayIdList_.begin(), displayIdList_.end(),
...
DisplayId DisplayManager::GetCallingAbilityDisplayId()
{
    ...
    std::lock_guard<std::mutex> lock(displayOperateMutex_);
    if (displayIdList_.empty()) {
        TLOGI(WmsLogTag::DMS_DM, "displayIdList is empty.");
        return displayId;
    }
    int displayCount = 0;
...
void DisplayManager::ShowDisplayIdList(bool isShowLog)
{
    ...
    if (isShowLog) {
        TLOGI(WmsLogTag::DMS_DM, "%{public}s]", oss.str().c_str());
    } else {
        TLOGD(WmsLogTag::DMS_DM, "%{public}s]", oss.str().c_str());
    }
}
...
sptr<Display> DisplayManager::GetDefaultDisplaySync(bool isFromNapi)
{
    if (isFromNapi) {
        ...
        if (displayId != DISPLAY_ID_INVALID) {
            TLOGD(WmsLogTag::DMS_DM, "displayId:%{public}" PRIu64, displayId);
            display = pImpl_->GetDisplayById(displayId);
        }
        if (display != nullptr) {
            return display;
        } else {
            TLOGI(WmsLogTag::DMS_DM, "get displayId:%{public}" PRIu64" info nullptr.", displayId);
        }
    }
...
std::vector<sptr<Display>> DisplayManager::GetAllDisplays()
{
    ...
    for (auto displayId : displayIds) {
        ...
        if (display != nullptr) {
            ...
            TLOGE(WmsLogTag::DMS_DM, "display %" PRIu64" nullptr!", displayId);
        }
    }
    return res;
}
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "listener is nullptr.");
...
DMError DisplayManager::Impl::RegisterPrivateWindowListener(sptr<IPrivateWindowListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        ...
        TLOGI(WmsLogTag::DMS_DM, "privateWindowListener register success");
...
DMError DisplayManager::Impl::RegisterPrivateWindowListChangeListener(sptr<IPrivateWindowListChangeListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        ...
        TLOGI(WmsLogTag::DMS_DM, "privateWindowListChangeListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "RegisterDisplayListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
{
    if (listener == nullptr) {
        TLOGW(WmsLogTag::DMS_DM, "UnregisterDisplayListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterDisplayPowerEventListener(sptr<IDisplayPowerEventListener> listener)
{
    ...
    TLOGD(WmsLogTag::DMS_DM, "RegisterDisplayPowerEventListener end");
    return ret;
}
...
DMError DisplayManager::Impl::UnregisterDisplayPowerEventListener(sptr<IDisplayPowerEventListener> listener)
{
    ...
    TLOGD(WmsLogTag::DMS_DM, "UnregisterDisplayPowerEventListener end");
    return ret;
}
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "IFoldAngleListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterFoldAngleListener(sptr<IFoldAngleListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "RegisterFoldAngleListener failed !");
        foldAngleListenerAgent_ = nullptr;
    } else {
        TLOGD(WmsLogTag::DMS_DM, "IFoldAngleListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "ICaptureStatusListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterCaptureStatusListener(sptr<ICaptureStatusListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGE(WmsLogTag::DMS_DM, "RegisterCaptureStatusListener failed !");
        captureStatusListenerAgent_ = nullptr;
    } else {
        TLOGD(WmsLogTag::DMS_DM, "ICaptureStatusListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "UnregisterCaptureStatusListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "IFoldStatusListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterFoldStatusListener(sptr<IFoldStatusListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "RegisterFoldStatusListener failed !");
        foldStatusListenerAgent_ = nullptr;
    } else {
        TLOGD(WmsLogTag::DMS_DM, "IFoldStatusListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "UnregisterFoldStatusListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "IDisplayUpdateListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterDisplayUpdateListener(sptr<IDisplayUpdateListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "RegisterDisplayUpdateListener failed !");
        displayUpdateListenerAgent_ = nullptr;
    } else {
        TLOGI(WmsLogTag::DMS_DM, "IDisplayUpdateListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "UnregisterDisplayUpdateListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "IDisplayModeListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterDisplayModeListener(sptr<IDisplayModeListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "RegisterDisplayModeListener failed !");
        displayModeListenerAgent_ = nullptr;
    } else {
        TLOGD(WmsLogTag::DMS_DM, "IDisplayModeListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "UnregisterPrivateWindowListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "IScreenMagneticStateListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterScreenMagneticStateListener(sptr<IScreenMagneticStateListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "RegisterScreenMagneticStateListener failed !");
        screenMagneticStateListenerAgent_ = nullptr;
    } else {
        TLOGD(WmsLogTag::DMS_DM, "IScreenMagneticStateListener register success");
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "UnregisterScreenMagneticStateListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
{
    if (listener == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "RegisterAvailableAreaListener listener is nullptr.");
        return DMError::DM_ERROR_NULLPTR;
    }
...
DMError DisplayManager::Impl::RegisterAvailableAreaListener(sptr<IAvailableAreaListener> listener)
{
    ...
    if (ret != DMError::DM_OK) {
        TLOGW(WmsLogTag::DMS_DM, "RegisterAvailableAreaListener failed !");
        availableAreaListenerAgent_ = nullptr;
    } else {
        TLOGD(WmsLogTag::DMS_DM, "IAvailableAreaListener register success");
...
{
    TLOGI(WmsLogTag::DMS_DM, "NotifyScreenshot trigger:[%{public}s] displayId:%{public}" PRIu64" size:%{public}zu",
...
{
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]NotifyDisplayPowerEvent event:%{public}u, status:%{public}u, size:%{public}zu",
...
{
    TLOGD(WmsLogTag::DMS_DM, "state:%{public}u", state);
    ...
    TLOGW(WmsLogTag::DMS_DM, "callback_ target is not set!");
}
...
{
    if (displayInfo == nullptr) {
        TLOGW(WmsLogTag::DMS_DM, "displayInfo is null");
        return false;
    }
    DisplayId displayId = displayInfo->GetDisplayId();
    TLOGD(WmsLogTag::DMS_DM, "displayId:%{public}" PRIu64".", displayId);
    ...
    if (iter != displayMap_.end() && iter->second != nullptr) {
        TLOGD(WmsLogTag::DMS_DM, "display Info Updated: %{public}s",
    ...
    if (display == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "malloc display failed");
        return false;
    }
...
{
    if (displayInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "displayInfo nullptr.");
        return "";
    }
    std::ostringstream oss;
...
{
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]WakeUpBegin start, reason:%{public}u", reason);
    return SingletonContainer::Get<DisplayManagerAdapter>().WakeUpBegin(reason);
}

bool DisplayManager::WakeUpEnd()
{
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]WakeUpEnd start");
...
{
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]SetScreenPowerById start");
...
{
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]state:%{public}u", state);
    bool ret = true;
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        if (displayStateCallback_ != nullptr || callback == nullptr) {
            if (displayStateCallback_ != nullptr) {
                TLOGI(WmsLogTag::DMS_DM, "[UL_POWER]previous callback not called, "
                    "the displayStateCallback_ is not null");
            }
            if (callback == nullptr) {
                TLOGI(WmsLogTag::DMS_DM, "[UL_POWER]Invalid callback received");
            }
            return false;
        }
...
{
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]TryToCancelScreenOff start");
...
{
    TLOGI(WmsLogTag::DMS_DM, "[UL_POWER]ScreenId:%{public}" PRIu64", level:%{public}u,", screenId, level);
...
uint32_t DisplayManager::GetScreenBrightness(uint64_t screenId) const
{
    ...
    TLOGI(WmsLogTag::DMS_DM, "GetScreenBrightness screenId:%{public}" PRIu64", level:%{public}u,", screenId, level);
    return level;
}
...
void DisplayManager::NotifyDisplayEvent(DisplayEvent event)
{
    ...
    TLOGD(WmsLogTag::DMS_DM, "[UL_POWER]DisplayEvent:%{public}u", event);
...
{
    TLOGD(WmsLogTag::DMS_DM, "freeze display");
    if (displayIds.size() == 0) {
        TLOGE(WmsLogTag::DMS_DM, "freeze fail, num of display is 0");
        return false;
    }
    if (displayIds.size() > MAX_DISPLAY_SIZE) {
        TLOGE(WmsLogTag::DMS_DM, "freeze fail, displayIds size is bigger than %{public}u.", MAX_DISPLAY_SIZE);
        return false;
    }
...
{
    TLOGD(WmsLogTag::DMS_DM, "unfreeze display");
    if (displayIds.size() == 0) {
        TLOGE(WmsLogTag::DMS_DM, "unfreeze fail, num of display is 0");
        return false;
    }
    if (displayIds.size() > MAX_DISPLAY_SIZE) {
        TLOGE(WmsLogTag::DMS_DM, "unfreeze fail, displayIds size is bigger than %{public}u.", MAX_DISPLAY_SIZE);
        return false;
    }
...
{
    TLOGI(WmsLogTag::DMS_DM, "dms is died");
    if (g_dmIsDestroyed) {
        TLOGE(WmsLogTag::DMS_DM, "dm has been destructed, mutex_ is invalid");
        return;
    }
    std::lock_guard<std::recursive_mutex> lock(mutex_);
    displayManagerListener_ = nullptr;
...
{
    if (g_dmIsDestroyed) {
        TLOGE(WmsLogTag::DMS_DM, "dms is dying, pImpl_ is nullptr");
        return;
    }
    pImpl_->OnRemoteDied();
}
...
bool DisplayManager::Impl::ConvertScreenIdToRsScreenId(ScreenId screenId, ScreenId& rsScreenId)
{
    ...
    TLOGD(WmsLogTag::DMS_DM, "Convert ScreenId %{public}" PRIu64" To RsScreenId %{public}" PRIu64"",
        screenId, rsScreenId);
    return res;
}
...
DMError DisplayManager::ProxyForFreeze(std::set<int32_t> pidList, bool isProxy)
{
    ...
    TLOGI(WmsLogTag::DMS_DM, "pidList:%{public}s, isProxy: %{public}d", oss.str().c_str(), isProxy);
...
sptr<Display> DisplayManager::Impl::GetPrimaryDisplaySync()
{
    ...
    while (retryTimes < MAX_RETRY_NUM) {
        ...
        retryTimes++;
        TLOGW(WmsLogTag::DMS_DM, "get display info null, retry %{public}u times", retryTimes);
        std::this_thread::sleep_for(std::chrono::milliseconds(RETRY_WAIT_MS));
    }
    if (retryTimes >= MAX_RETRY_NUM || displayInfo == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "get display info failed");
        return nullptr;
    }
    ...
    DmErrorCode* errorCode)
{
    ...
    if (screenCapture == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "screen capture failed!");
        return nullptr;
    }
    ...
    DmErrorCode* errorCode)
{
    ...
    if (screenShot == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "get snapshot with option failed!");
        return nullptr;
    }
    return screenShot;
}
    ...
    const Media::Rect &rect, const Media::Size &size, int rotation, DmErrorCode* errorCode)
{
    ...
    if (screenShot == nullptr) {
        TLOGE(WmsLogTag::DMS_DM, "set snapshot with option failed!");
        return nullptr;
    }
    // check parameters
    int32_t oriHeight = screenShot->GetHeight();
