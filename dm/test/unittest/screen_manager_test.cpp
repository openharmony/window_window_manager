...
HWTEST_F(ScreenManagerTest, CreateAndDestroy01, TestSize.Level1)
{
    ...
    EXPECT_CALL(m->Mock(), CreateVirtualScreen(_, _, _)).Times(1).WillOnce(Return(SCREEN_ID_INVALID));
...
HWTEST_F(ScreenManagerTest, CreateAndDestroy02, TestSize.Level1)
{
    ...
    ScreenId validId = 0;
    std::unique_ptr<Mocker> m = std::make_unique<Mocker>();
    EXPECT_CALL(m->Mock(), CreateVirtualScreen(_, _, _)).Times(1).WillOnce(Return(validId));
...
HWTEST_F(ScreenManagerTest, MakeExpand_001, TestSize.Level1)
{
    ...
    std::unique_ptr<Mocker> m = std::make_unique<Mocker>();
    EXPECT_CALL(m->Mock(), CreateVirtualScreen(_, _, _)).Times(1).WillOnce(Return(virtualScreenId));
    EXPECT_CALL(m->Mock(), DestroyVirtualScreen(_)).Times(1).WillOnce(Return(DMError::DM_OK));
