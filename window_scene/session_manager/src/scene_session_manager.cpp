...
WMError SceneSessionManager::MinimizeByWindowId(const std::vector<int32_t>& windowIds)
{
    if (!SessionPermission::IsShellCall() && !SessionPermission::IsSACalling()) {
        TLOGE(WmsLogTag::WMS_SYSTEM, "The caller is neither Shell nor an SA.");
        return WMError::WM_ERROR_INVALID_PERMISSION;
    }
    if (windowIds.empty()) {
        TLOGE(WmsLogTag::WMS_SYSTEM, "The vector of windowid is empty.");
        return WMError::WM_ERROR_INVALID_PARAM;
    }
    taskScheduler_->PostAsyncTask([this, windowIds, where = __func__]() {
        for (const auto& persistentId : windowIds) {
            auto sceneSession = GetSceneSession(persistentId);
            if (sceneSession == nullptr) {
                TLOGE(WmsLogTag::WMS_SYSTEM, "could not find the window, persistentId:%{public}d",persistentId);
                continue;
            } else {
