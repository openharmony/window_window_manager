...
WSError SceneSessionManager::UpdateSessionDisplayId(int32_t persistentId, uint64_t screenId, WSRect winRect, bool isNotSessionRectWithDpiChange)
{
    ...
    if (sceneSession->pcFoldScreenController_ != nullptr &&
        PcFoldScreenManager::GetInstance().IsHalfFolded(displayId)){
...
WMError SceneSessionManager::GetCallingWindowInfo(CallingWindowInfo& callingWindowInfo)
{
    ...
    if (sceneSession->pcFoldScreenController_ != nullptr &&
        PcFoldScreenManager::GetInstance().IsHalfFolded(callingWindowInfo.displayId_)){
