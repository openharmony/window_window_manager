...
WSRect SceneSessionManager::GetRootSessionAvoidSessionRect(AvoidAreaType type)
{
    ...
    switch (type) {
        case AvoidAreaType::TYPE_SYSTEM: {
            sessionVector = GetSceneSessionVectorByTypeAndDisplayId(WindowType::WINDOW_TYPE_STATUS_BAR, displayId);
            break;
        }
        case AvoidAreaType::TYPE_KEYBOARD: {
            sessionVector = GetSceneSessionVectorByType(WindowType::WINDOW_TYPE_KEYBOARD_PANEL);
            break;
        }
        default: {
...
sptr<SceneSession> SceneSessionManager::GetSceneSessionByName(const ComparedSessionInfo& info)
{
    ...
    for (const auto &item : sceneSessionMap_) {
        ...
        if (info.isAtomicService_) {
            if ((sceneSession->GetSessionInfo().moduleName_.empty() ||
                sceneSession->GetSessionInfo().moduleName_ == info.moduleName_) &&
                (sceneSession->GetSessionInfo().abilityName_.empty() ||
...
uint32_t SceneSessionManager::GetLockScreenZOrder()
{
    ...
    for (const auto& [persistentId, session] : sceneSessionMap_) {
        if (session && session->IsScreenLockWindow()) {
            TLOGI(WmsLogTag::WMS_UIEXT, "UIExtOnLock: window %{public}d", persistentId);
    ...
    sptr<SceneSession>& scnSession, bool isNewActive)
{
    ...
    if (errCode != ERR_OK) {
        ...
        scnSession->NotifySessionExceptionInner(scnSessionInfo, true, false, true);
    ...
    sptr<WindowSessionProperty> property, sptr<ISession>& session, sptr<IRemoteObject> token)
{
    ...
    auto task = [this, sessionStage, eventChannel, surfaceNode, property, &session, token, pid, uid]() {
        ...
        TLOGI(WmsLogTag::WMS_RECOVER, "callingWindowId = %{public}" PRIu32, property->GetCallingSessionId());
...
WSError SceneSessionManager::ProcessBackEvent()
{
    auto task = [this]() {
        ...
        if (needBlockNotifyFocusStatusUntilForeground_ || !session->IsSessionValid()) {
    ...
    auto task = [this, weakSceneSession, requireLock, &screenLockPrefix, &screenLock]() {
        ...
        if (requireLock && screenLock == nullptr) {
            ...
            screenLock = PowerMgr::PowerMgrClient::GetInstance().CreateRunningLock(
                screenLockPrefix + std::to_string(scnSession->GetPersistentId()),
        ...
        auto curScreenId = scnSession->GetSessionInfo().screenId_;
        ...
        bool shouldLock = requireLock && IsSessionVisibleForeground(scnSession) && sourceMode != ScreenSourceMode::SCREEN_UNIQUE;
        TLOGNI(WmsLogTag::WMS_ATTRIBUTE,
            "keep screen on: [%{public}s, %{public}d, %{public}d, %{public}d, %{public}d, %{public}" PRIu64 ", %{public}d]",
            scnSession->GetWindowName().c_str(), scnSession->GetSessionState(),
            scnSession->IsVisible(), requireLock, shouldLock, curScreenId, sourceMode);
...
{
    TLOGD(WmsLogTag::WMS_FOCUS, "RequestSessionFocusImmediately, id: %{public}d", persistentId);
    // base block
...
WSError SceneSessionManager::RequestSessionFocus(int32_t persistentId, bool byForeground, FocusChangeReason reason)
{
    ...
    WSError checkRet = RequestSessionFocusCheck(sceneSession, persistentId, byForeground, reason);
...
WSError SceneSessionManager::RequestAllAppSessionUnfocusInner()
{
    ...
    needBlockNotifyUnfocusStatus_ = needBlockNotifyFocusStatusUntilForeground_;
    needBlockNotifyFocusStatusUntilForeground_ = false;
    return ShiftFocus(nextSession, true, FocusChangeReason::WIND);
}

WSError SceneSessionManager::RequestSessionFocusCheck(sptr<SceneSession>& sceneSession, int32_t persistentId,
    ...
    const sptr<SceneSession>& currSceneSession, bool isProactiveUnfocus)
{
    ...
    bool needBlockHighlightNotify = needBlockNotifyFocusStatusUntilForeground_;
    ...
    for (const auto& item : sceneSessionMap_) {
        ...
        if ((isForeground || IsSystemWindowVisible) && isPrivate) {
            if (!sceneSession->GetSessionInfo().bundleName_.empty()) {
                ...
                TLOGD(WmsLogTag::WMS_MAIN, "bundle name is empty, wid = %{public}d.", item.first);
...
WSError SceneSessionManager::ProcessSubWindowRequestFocusImmdediately(sptr<SceneSession>& sceneSession)
{
    if (sceneSession == nullptr) {
        TLOGD(WmsLogTag::WMS_FOCUS, "session is nullptr");
        return WSError::WS_DO_NOTHING;
    ...
    auto conditionFunc =  [this](const sptr<SceneSession>& iter) {
        return iter && iter->GetPersistentId() == focusedSessionId_;
    };
    if (std::find_if(subSessionVec.begin(), subSessionVec.end(), std::move(conditionFunc)) != subSessionVec.end()) {
        TLOGD(WmsLogTag::WMS_SUB, "sub window id: %{public}d has been focused!", focusedSessionId_);
        return WSError::WS_OK;
    }
    WSError ret = WSError::WS_DO_NOTHING;
    ...
    const std::vector<std::pair<uint64_t, WindowVisibilityState>>& currVisibleData)
{
    if (WindowHelper::IsMainWindow(session->GetWindowType()) &&
        ...
        for (const auto& subSession : subSessions) {
            ...
            if (session->GetCallingPid() != subSession->GetCallingPid() &&
                (subSession->IsSessionForeground() || GetSessionRSVisible(subSession, currVisibleData))) {
...
bool SceneSessionManager::IsSessionClearable(sptr<SceneSession> scnSession)
{
    ...
    if (sessionInfo.abilityInfo->excludeFromMissions && !scnSession->IsAnco()) {
    ...
    const sptr<IRemoteObject>& token, uint64_t surfaceNodeId, bool isConstrainedModal)
{
    ...
    auto task = [this, sessionStage, token, surfaceNodeId, isConstrainedModal, pid, callingTokenId]() {
        ...
        if (usage == UIExtensionUsage::MODAL) {
...
WMError SceneSessionManager::CloseTargetFloatWindow(const std::string& bundleName)
{
    ...
    auto task = [this, bundleName] {
        if (closeTargetFloatWindowFunc_) {
            TLOGNI(WmsLogTag::WMS_MULTI_WINDOW, "bundleName:%{public}s", bundleName.c_str());
    ...
    taskScheduler_->PostAsyncTask([this, abilityInfo, isLock, where = __func__] {
        ...
        if (!mainSessions.empty()) {
            for (const auto& mainSession : mainSessions) {
                TLOGNI(WmsLogTag::WMS_LIFE, "%{public}s, set isLocked %{public}d, persistentId:%{public}d",
                    where, isLock, mainSession->GetPersistentId());
