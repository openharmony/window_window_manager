...
static WMError FindMainWindowIdInner(const std::map<int32_t, sptr<SceneSession>>& sceneSessionMap,
    const uint32_t windowId, uint32_t& mainWindowId)
{
    ...
    bool isSystemMainWindow = WindowHelper::IsSystemWindow(parentSession->GetWindowType()) &&
        parentSession->GetWindowId() == INVALID_SESSION_ID;
    if (WindowHelper::IsMainWindow(parentSession->GetWindowType()) || isSystemMainWindow) {
        TLOGI(WmsLogTag::WMS_SUB, "find main window, id:%{public}u", parentSession->GetWindowId());
        mainWindowId = parentSession->GetWindowId();
    } else if (WindowHelper::IsSubWindow(parentSession->GetWindowType()) ||
        WindowHelper::IsDialogWindow(parentSession->GetWindowType())) {
        return FindMainWindowIdInner(sceneSessionMap, parentSession->GetParentPersistentId(), mainWindowId);
    } else {
...
WMError SceneSessionManager::FindMainWindowId(const uint32_t windowId, uint32_t& mainWindowId)
{
    ...
    std::unique_lock<std::shared_mutex> lock(sceneSessionMapMutex_);
    return FindMainWindowIdInner(sceneSessionMap_, windowId, mainWindowId);
}
