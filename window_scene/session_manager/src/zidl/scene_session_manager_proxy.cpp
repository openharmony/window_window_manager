    ...
    std::shared_ptr<RSSurfaceNode>& surfaceNode, sptr<IRemoteObject> token)
{
    ...
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, failed to write interface token"};
    }
    if (!data.WriteRemoteObject(sessionStage->AsObject())) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write ISessionStage failed!");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to write the ISessionStage remote object."};
    }
    if (!data.WriteRemoteObject(eventChannel->AsObject())) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write IWindowEventChannel failed!");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to write IWindowEventChannel."};
    }
    if (!data.WriteUint64(nodeId)) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write nodeId failed");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, failed to write node ID."};
    }
    if (!property || !data.WriteStrongParcelable(property)) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write property failed");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, failed to write property. "};
    }
    if (token != nullptr) {
        if (!data.WriteRemoteObject(token)) {
            return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, failed to write token."};
        }
    }
    ...
    if (remote == nullptr) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, the remote binder object is null."};
    }
    ...
    if (remote->SendRequest(static_cast<uint32_t>(
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, IPC SendRequest invocation failed."};
    }
    ...
    if (sessionObject == nullptr) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to read the session remote object."};
    }
    ...
    if (renderSessionObject == nullptr) {
        TLOGE(WmsLogTag::WMS_LIFE, "Read RemoteObject failed");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to read the render session remote object."};
    }
    ...
    if (!surfaceNode) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to read the RSSurfaceNode parcel data."};
    }
    ...
    if (!reply.ReadUint32(level)) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to read the subwindow level value"};
    }
    property->SetSubWindowLevel(level);
    uint64_t displayId = 0;
    if (!reply.ReadUint64(displayId)) {
        TLOGE(WmsLogTag::WMS_LIFE, "Read displayId failed");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, failed to read the display ID."};
    }
    ...
    if (!reply.ReadUint32(windowType)) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to read the window type value."};
    }
    ...
    if (!reply.ReadBool(isSystemCalling)) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Internal task error, "
            "failed to read the system‑calling flag."};
    }
...
WSErrorResult SceneSessionManagerProxy::DestroyAndDisconnectSpecificSession(const int32_t persistentId)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "WriteInterfaceToken failed"};
    }
    if (!data.WriteInt32(persistentId)) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write persistentId failed");
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Write persistentId failed"};
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
        TLOGE(WmsLogTag::WMS_LIFE, "remote is null");
    ...
    const sptr<IRemoteObject>& callback)
{
    ...
    if (callback == nullptr || !data.WriteRemoteObject(callback)) {
        ...
        return WSErrorResult{WSError::WS_ERROR_IPC_FAILED, "Write callback failed"};
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
        TLOGE(WmsLogTag::WMS_LIFE, "remote is null");
...
WMError SceneSessionManagerProxy::UpdateSessionScreenshotListener(int32_t persistentId, bool haveListener)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    auto reqErrCode = remote->SendRequest(
        static_cast<uint32_t>(SceneSessionManagerMessage::TRANS_ID_UPDATE_SESSION_SCREENSHOT_LISTENER),
        data, reply, option);
...
WMError SceneSessionManagerProxy::GetWindowStateSnapshot(int32_t persistentId, std::string& winStateSnapshotJsonStr)
{
    ...
    if (!data.WriteString(winStateSnapshotJsonStr)) {
    ...
    auto reqErrCode = remote->SendRequest(
        static_cast<uint32_t>(SceneSessionManagerMessage::TRANS_ID_GET_WINDOW_STATE_SNAPSHOT),
        data, reply, option);
    if (reqErrCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "send request failed, errCode: %{public}d", reqErrCode);
        return WMError::WM_ERROR_IPC_FAILED;
    }
    if (!reply.ReadString(winStateSnapshotJsonStr)) {
    ...
    bool needStartCaller, bool isFromBroker)
{
    ...
    if (remote->SendRequest(static_cast<uint32_t>(
        SceneSessionManagerMessage::TRANS_ID_TERMINATE_SESSION_NEW),
        data, reply, option) != ERR_NONE) {
        WLOGFE("SendRequest failed");
        return WSError::WS_ERROR_IPC_FAILED;
    }
...
WMError SceneSessionManagerProxy::SetWindowSnapshotSkip(int32_t windowId, bool isSkip)
{
    ...
    if (!data.WriteBool(isSkip)) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "write isSkip failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
    ...
    auto reqErrCode = remote->SendRequest(static_cast<uint32_t>(
        SceneSessionManagerMessage::TRANS_ID_SET_WINDOW_SNAPSHOT_SKIP), data, reply, option);
    if (reqErrCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "send request failed, errCode: %{public}d", reqErrCode);
        return WMError::WM_ERROR_IPC_FAILED;
    }
...
WMError SceneSessionManagerProxy::SetScreenWatermarkImage(const std::shared_ptr<Media::PixelMap>& pixelMap,
    uint32_t priority, std::string& bundleName)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    if (!data.WriteUint32(priority)) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "write priority failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
    ...
    auto reqErrCode = remote->SendRequest(
        static_cast<uint32_t>(SceneSessionManagerMessage::TRANS_ID_SET_SCREEN_WATERMARK_IMAGE), data, reply, option);
    if (reqErrCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "send request failed, errCode: %{public}d", reqErrCode);
        return WMError::WM_ERROR_IPC_FAILED;
    }
...
WMError SceneSessionManagerProxy::CleanScreenWatermarkImage(const std::shared_ptr<Media::PixelMap>& pixelMap)
{
    ...
    auto reqErrCode = remote->SendRequest(
        static_cast<uint32_t>(SceneSessionManagerMessage::TRANS_ID_CLEAN_SCREEN_WATERMARK_IMAGE), data, reply, option);
    if (reqErrCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "send request failed, errCode: %{public}d", reqErrCode);
        return WMError::WM_ERROR_IPC_FAILED;
    }
...
WMError SceneSessionManagerProxy::RecoverScreenWatermarkImage(const std::string& bundleName, uint32_t priority)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    auto reqErrCode = remote->SendRequest(
        static_cast<uint32_t>(SceneSessionManagerMessage::TRANS_ID_RECOVER_SCREEN_WATERMARK_IMAGE),
        data, reply, option);
    if (reqErrCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "send request failed, errCode: %{public}d", reqErrCode);
        return WMError::WM_ERROR_IPC_FAILED;
...
WMError SceneSessionManagerProxy::MinimizeAllAppWindows(DisplayId displayId, int32_t excludeWindowId)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    if (!data.WriteInt32(excludeWindowId)) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write excludeWindowId failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
...
WMError SceneSessionManagerProxy::NotifySupportRotationRegistered()
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        TLOGE(WmsLogTag::WMS_ROTATION, "Write interfaceToken failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    sptr remote = Remote();
    if (remote == nullptr) {
    ...
    if (remote->SendRequest(static_cast<uint32_t>(SceneSessionManagerMessage::TRANS_ID_SUPPORT_ROTATION_REGISTERED),
        data, reply, option) != ERR_NONE) {
