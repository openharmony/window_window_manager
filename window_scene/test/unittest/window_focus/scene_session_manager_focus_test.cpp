...
/**
 * @tc.name: GetTopNearestBlockingFocusSession_TopmostMainWindow
 * @tc.desc: Topmost main window does not block
 * @tc.type: FUNC
 */
HWTEST_F(SceneSessionManagerFocusTest, GetTopNearestBlockingFocusSession_TopmostMainWindow, TestSize.Level1)
{
    ASSERT_NE(ssm_, nullptr);
    ssm_->sceneSessionMap_.clear();

    SessionInfo info;
    info.bundleName_ = "TopmostApp";
    sptr<SceneSession> topmostSession = sptr<SceneSession>::MakeSptr(info, nullptr);
    topmostSession->property_->SetWindowType(WindowType::WINDOW_TYPE_APP_MAIN_WINDOW);
    topmostSession->SetTopmost(true);
    topmostSession->zOrder_ = 100;
    topmostSession->isVisible_ = true;
    topmostSession->SetSessionState(SessionState::STATE_FOREGROUND);

    ssm_->sceneSessionMap_.insert({ 1, topmostSession });

    auto ret = ssm_->GetTopNearestBlockingFocusSession(DEFAULT_DISPLAY_ID, 50, true);
...
/**
 * @tc.name: GetTopNearestBlockingFocusSession_TopmostSubWindow
 * @tc.desc: Sub window of topmost main window does not block
 * @tc.type: FUNC
 */
HWTEST_F(SceneSessionManagerFocusTest, GetTopNearestBlockingFocusSession_TopmostSubWindow, TestSize.Level1)
{
    ASSERT_NE(ssm_, nullptr);
    ssm_->sceneSessionMap_.clear();
    ...
    sptr<SceneSession> mainSession = sptr<SceneSession>::MakeSptr(mainInfo, nullptr);
    mainSession->property_->SetWindowType(WindowType::WINDOW_TYPE_APP_MAIN_WINDOW);
    mainSession->SetTopmost(true);
    mainSession->zOrder_ = 100;
    ...
    subSession->SetParentSession(mainSession);
    subSession->zOrder_ = 150;
    subSession->isVisible_ = true;
    subSession->SetSessionState(SessionState::STATE_FOREGROUND);

    ssm_->sceneSessionMap_.insert({ 1, mainSession });
    ssm_->sceneSessionMap_.insert({ 2, subSession });
...
/**
 * @tc.name: GetTopNearestBlockingFocusSession_BlockingWindow
 * @tc.desc: Normal blocking window is returned
 * @tc.type: FUNC
 */
HWTEST_F(SceneSessionManagerFocusTest, GetTopNearestBlockingFocusSession_BlockingWindow, TestSize.Level1)
{
    ASSERT_NE(ssm_, nullptr);
    ssm_->sceneSessionMap_.clear();

    SessionInfo info;
    info.bundleName_ = "BlockingApp";
    info.isSystem_ = true;
    sptr<SceneSession> blockingSession = sptr<SceneSession>::MakeSptr(info, nullptr);
    blockingSession->property_->SetWindowType(WindowType::WINDOW_TYPE_PANEL);
    blockingSession->blockingFocus_ = true;
    blockingSession->zOrder_ = 100;
    blockingSession->isVisible_ = true;
    blockingSession->SetSessionState(SessionState::STATE_FOREGROUND);

    ssm_->sceneSessionMap_.insert({ 1, blockingSession });

    auto ret = ssm_->GetTopNearestBlockingFocusSession(DEFAULT_DISPLAY_ID, 50, false);
    EXPECT_NE(ret, nullptr);
    EXPECT_EQ(ret->GetPersistentId(), 1);

    ssm_->sceneSessionMap_.clear();
}

/**
 * @tc.name: GetTopNearestBlockingFocusSession_ZOrderFilter
 * @tc.desc: Window with ZOrder <= target is not returned
 * @tc.type: FUNC
 */
HWTEST_F(SceneSessionManagerFocusTest, GetTopNearestBlockingFocusSession_ZOrderFilter, TestSize.Level1)
{
    ASSERT_NE(ssm_, nullptr);
    ssm_->sceneSessionMap_.clear();
    ...
    blockingSession->property_->SetWindowType(WindowType::WINDOW_TYPE_PANEL);
    blockingSession->blockingFocus_ = true;
    blockingSession->zOrder_ = 30;  // below target
    blockingSession->isVisible_ = true;
    blockingSession->SetSessionState(SessionState::STATE_FOREGROUND);
