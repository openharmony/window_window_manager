...
/**
 * @tc.name: HandleMinimizeAllWindow
...
HWTEST_F(SceneSessionManagerStubLifecycleTest, HandleMinimizeAllWindowWhenWriteValidDataThenReturnNone, TestSize.Level1)
{
    ...
    MessageParcel reply;
    data.WriteBool(true);
    EXPECT_EQ(stub_->HandleMinimizeAllWindow(data, reply), ERR_INVALID_DATA);

    data.WriteUint64(0);
    EXPECT_EQ(stub_->HandleMinimizeAllWindow(data, reply), ERR_NONE);
}
