 ...
 * @tc.type: FUNC
 */
HWTEST_F(ScenePersistenceTest, GetSnapshotFilePath, Function | SmallTest | Level3)
{
    std::string path = ScenePersistence->GetSnapshotFilePath();
    ASSERT_NE("0", path);

    std::string path2 = ScenePersistence->GetSnapshotFilePathFromAce();
    ASSERT_NE("0", path2);
}
