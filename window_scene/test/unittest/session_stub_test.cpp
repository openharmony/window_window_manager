...
/*
 * @tc.name: HandleUpdateNavigationAvoidArea
 * @tc.desc: Verify HandleUpdateNavigationAvoidArea with invalid and valid inputs
 * @tc.type: FUNC
 */
HWTEST_F(SessionStubTest, HandleUpdateNavigationAvoidArea, TestSize.Level1)
{
    sptr<SessionStubMocker> session = sptr<SessionStubMocker>::MakeSptr();
    ...
    // Case 2: Success
    {
        ...
        EXPECT_CALL(*session, UpdateNavigationAvoidArea(enable)).Times(1);
        EXPECT_EQ(session->ProcessRemoteRequest(code, data, reply, option), ERR_NONE);
    }
    MessageParcel data;
    MessageParcel reply;
