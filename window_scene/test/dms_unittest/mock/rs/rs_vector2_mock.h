...
namespace Rosen {
template<typename T>
inline constexpr bool ROSEN_EQ2(const T& x, const T& y)
{
...
inline bool Vector2<T>::operator==(const Vector2& other) const
{
    ...
    return (ROSEN_EQ2<T>(data_[0], oData[0])) && (ROSEN_EQ2<T>(data_[1], oData[1]));
}
...
bool Vector2<T>::IsNearEqual(const Vector2& other, T threshold) const
{
    ...
    return (ROSEN_EQ2<T>(data_[0], otherData[0], threshold)) && (ROSEN_EQ2<T>(data_[1], otherData[1], threshold));
}
