...
public:
    ...
    RState GetPluralStringByIdFormat(std::string& outValue, const char* name, ...) {return RState::ERROR;};
    RState GetPluralStringByNameFormat(std::string& outValue, const char* name, intquantity, ...) {
        return RState::ERROR;
    };
    ...
    RState GetMediaBase64DataByName(const char* name, std::string& outValue, uint32_t density) {return RState::ERROR;};
    ...
    RState GetRawFileList(std::string& rawDirPath, std::vector<std::string>& rawfileList) {return RState::ERROR;};
    RState GetDrawableInfoById(uint32_t id, std::string& type, size_t& len, std::unique_ptr<uint8_t[]>& outValue,
        uint32_t density) {return RState::ERROR;};
    RState GetDrawableInfoByName(const char* name, std::string& type, size_t& len, std::unique_ptr<uint8_t[]>& outValue,
        uint32_t density) {return RState::ERROR;};
    bool AddResource(const std::string& path, const std::vector<std::string>& overlayPaths) {return true;};
    bool RemoveResource(const std::string& path, const std::vector<std::string>& overlayPaths) {return true;};
    RState GetStringFormatById(uint32_t id, std:: string& outValue,
        std::vector<std::tuple<NapiValueType, std::string>>& jsParams) {return RState::ERROR};
    ...
    RState GetRawFdNdkFromHap(const std::string& rawFileName, RawFileDescriptor& descriptor) {return RState::ERROR};
    ...
    RState GetDrawableInfoById(uint32_t id, std::tuple<std::string, size_t, std::string>& drawableInfo,
        std::unique_ptr<uint8_t[]>& outValue, uint32_t iconType, uint32_t density) {return RState::ERROR};
    ...
    RState GetThemeIcons(uint32_t resId, std::pair<std::unique_ptr<uint8_t[]>, size_t>& foregroundInfo,
        ...
        const std::string& abilityName) {return RState::ERROR};
    std::string GetThemeMask() {return "";};
    bool HasIconTheme(const std::string& bundleName) {return true;};
    RState GetOtherIconsInfo(const std::string & iconName, std::unique_ptr<uint8_t[]>& outValue,
        size_t& len, bool isGlobalMask) {return RState::ERROR};
    RState IsRawDirFromHap(const std::string& pathName, bool& outValue) {return RState::ERROR};
    ...
    RState GetFormatPluralStringById(std::string& outValue, uint32_t id, int quantity,
        std::vector<std::tuple<ResourceManager::NapiValueType, std::string>>& jsParams) {return RState::ERROR};
