...
WSError SceneSession::PendingSessionActivation(const sptr<AAFwk::SessionInfo> abilitySessionInfo)
{
    ...
    auto task = [weakThis = wptr(this), abilitySessionInfo]() {
        ...
        if (!isPC && (session->GetAbilityInfo() != nullptr) && WindowHelper::IsMainWindow(session->GetWindowType())) {
            ...
            auto startAbilityBackground = 
                SessionPermission::VerifyCallingPermission(callingTokenId, "ohos.permission.START_ABILITIES_FROM_BACKGROUND") ||
                SessionPermission::VerifyCallingPermission(callingTokenId, "ohos.permission.START_ABILIIES_FROM_BACKGROUND");
            ...
            if (sessionState != SessionState::SessionState::STATE_ACTIVE &&
                ...
                TLOGW(WmsLogTag::WMS_LIFE,
                    "start ability invalid, window state: %{public}d, startAbilityBackground:%{public}u, hasContinuousTask: %{public}u",
