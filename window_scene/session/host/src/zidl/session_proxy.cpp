...
bool WriteAbilitySessionInfoBasic(MessageParcel& data, sptr<AAFwk::SessionInfo> abilitySessionInfo)
{
    ...
    if (!data.WriteParcelable(&(abilitySessionInfo->want)) ||
        ...
        !data.WriteInt32(abilitySessionInfo->specifiedReason) ||
...
/**
 * @brief Write a Rect to MessageParcel.
 *
 * @param data The MessageParcel to write to.
 * @param rect The Rect to write.
 * @return true if writing succeeds, false otherwise.
 */
bool WriteRect(MessageParcel& data, const Rect& rect)
{
    if (!data.WriteInt32(rect.posX_)) {
        TLOGE(WmsLogTag::DEFAULT, "Failed to write posX.");
        return false;
    }
    if (!data.WriteInt32(rect.posY_)) {
        TLOGE(WmsLogTag::DEFAULT, "Failed to write posY.");
        return false;
    }
    if (!data.WriteUint32(rect.width_)) {
        TLOGE(WmsLogTag::DEFAULT, "Failed to write width.");
        return false;
    }
    if (!data.WriteUint32(rect.height_)) {
        TLOGE(WmsLogTag::DEFAULT, "Failed to write height.");
        return false;
    }
    return true;
}
...
WSError SessionProxy::Show(sptr<WindowSessionProperty> property)
{
    ...
    int sendCode = remote->SendRequest(static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_SHOW),
        data, reply, option);
    if (sendCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_LIFE, "SendRequest failed, code: %{public}d", sendCode);
        return WSError::WS_ERROR_IPC_FAILED;
...
WSError SessionProxy::Hide()
{
    ...
    int sendCode = remote->SendRequest(static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_HIDE),
        data, reply, option);
    if (sendCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_LIFE, "SendRequest failed, code: %{public}d", sendCode);
        return WSError::WS_ERROR_IPC_FAILED;
...
WSError SessionProxy::PendingSessionActivation(sptr<AAFwk::SessionInfo> abilitySessionInfo)
{
    ...
    if (!data.WriteBool(abilitySessionInfo->isTargetPlugin)) {
        TLOGE(WmsLogTag::WMS_LIFE, "Write isTargetPlugin failed");
        return WSError::WS_ERROR_IPC_FAILED;
    }
...
WMError SessionProxy::RestoreFloatMainWindow(const std::shared_ptr<AAFwk::WantParams>& wantParams)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    if (remote->SendRequest(
        static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_RESTORE_FLOAT_MAIN_WINDOW),
        data, reply, option) != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_LIFE, "SendRequest failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
...
WSError SessionProxy::RaiseMainWindowAboveTarget(int32_t targetId)
{
    ...
    if (!data.WriteInt32(targetId)) {
        TLOGE(WmsLogTag::WMS_HIERARCHY, "write targetId failed");
        return WSError::WS_ERROR_IPC_FAILED;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
...
WMError SessionProxy::SetPreferredOrientationWithResult(
    Orientation orientation, uint32_t promiseId, bool needAnimation)
{
    ...
    if (remote->SendRequest(static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_SET_PREFERRED_ORIENTATION_WITH_RESULT),
        data, reply, option) != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_ROTATION, "SendRequest failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
...
WSError SessionProxy::NotifySurfaceNodeAlphaUpdate(float alpha)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
...
WMError SessionProxy::GetSelectMode(SelectMode& selectMode)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    uint32_t requestCode = static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_GET_SELECT_MODE);
    if (remote->SendRequest(requestCode, data, reply, option) != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "SendRequest failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    uint32_t selectModeValue = 0;
    if (!reply.ReadUint32(selectModeValue)) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "read selectMode failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    selectMode = static_cast<SelectMode>(selectModeValue);
    int32_t ret = 0;
    if (!reply.ReadInt32(ret)) {
...
void SessionProxy::NotifyWindowStatusDidChangeAfterShowWindow()
{
    ...
    uint32_t requestCode = static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_NOTIFY_WINDOW_STATUS_AFTER_SHOW_WINDOW);
    int ret = remote->SendRequest(requestCode, data, reply, option);
    if (ret != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "SendRequest failed, ret: %{public}d", ret);
    }
}
...
WMError SessionProxy::StartMovingWithOptions(const StartMovingOptions& options)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "Failed to write interface token.");
    ...
    if (!data.WriteBool(options.needFocused)) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "Failed to write needFocused.");
        return WMError::WM_ERROR_IPC_FAILED;
    }

    if (!WriteRect(data, options.avoidRect)) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "Failed to write avoidRect.");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    ...
    MessageOption option(MessageOption::TF_SYNC);
    int sendRet = remote->SendRequest(
        static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_START_MOVING_WITH_OPTIONS), data, reply, option);
    if (sendRet != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "Failed to send request, error = %{public}d", sendRet);
        return WMError::WM_ERROR_IPC_FAILED;
    }
...
WMError SessionProxy::IsPiPActive(bool& status)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
...
void SessionProxy::NotifyFloatViewPrepareClose()
{
    TLOGI(WmsLogTag::WMS_SYSTEM, "NotifyFloatViewPrepareClose");
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_ASYNC);
    if (!data.WriteInterfaceToken(GetDescriptor())) {
...
WMError SessionProxy::UpdateFloatView(const OHOS::Rosen::FloatViewTemplateInfo& fvTemplateInfo)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    if (!data.WriteParcelable(&fvTemplateInfo)) {
        TLOGE(WmsLogTag::WMS_SYSTEM, "write fvTemplateInfo failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
    ...
    auto errCode = remote->SendRequest(static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_UPDATE_FLOAT_VIEW),
                                       data, reply, option);
    if (errCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_SYSTEM, "SendRequest failed code: %{public}d", errCode);
        return WMError::WM_ERROR_IPC_FAILED;
...
WMError SessionProxy::RestoreFloatViewMainWindow(const std::shared_ptr<AAFwk::WantParams>& wantParams)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
...
WMError SessionProxy::NotifySplitRatioChanged(float newRatio)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    if (!data.WriteFloat(newRatio)) {
        TLOGE(WmsLogTag::WMS_COMPAT, "write newRatio failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    auto remote = Remote();
    if (remote == nullptr) {
    ...
    if (remote->SendRequest(static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_NOTIFY_SPLIT_RATIO_CHANGED),
        data, reply, option) != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_COMPAT, "SendRequest failed");
        return WMError::WM_ERROR_IPC_FAILED;
        }
...
WSError SessionProxy::NotifyPageEnable(const std::string& action, const std::string& message)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_ASYNC);
    ...
    if (!data.WriteString(action) || !data.WriteString(message)) {
        TLOGE(WmsLogTag::WMS_COMPAT, "Write action or message failed");
        return WSError::WS_ERROR_IPC_FAILED;
    }
    ...
    int sendCode = remote->SendRequest(
        static_cast<uint32_t>(SessionInterfaceCode::TRANS_ID_NOTIFY_PAGE_ENABLE),
        data, reply, option);
    if (sendCode != ERR_NONE) {
        TLOGE(WmsLogTag::WMS_COMPAT, "SendRequest failed");
        return WSError::WS_ERROR_IPC_FAILED;
