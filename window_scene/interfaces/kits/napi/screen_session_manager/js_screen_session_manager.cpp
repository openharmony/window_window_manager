...
napi_value JsScreenSessionManager::OnUnRegisterCallback(napi_env env, const napi_callback_info info)
{
    ...
    if (callbackType == ON_TENT_MODE_CHANGE_CALLBACK) {
        UnRegisterTentModeCallback(env, callbackRef);
    } else {
        TLOGE(WmsLogTag::DMS, "Unsupported callback type: %{public}s.", callbackType.c_str());
        napi_throw(env, CreateJsError(env, static_cast<int32_t>(WSErrorCode::WS_ERROR_INVALID_PARAM)));
    }
...
napi_value JsScreenSessionManager::OnGetScreenSnapshotWithAllWindows(napi_env env, const napi_callback_info info)
{
    ...
    for (size_t i = 0; i < ARGC_TWO; i++) {
        if (!ConvertFromJsValue(env, argv[i + 1], scaleParam[i])) {
            TLOGE(WmsLogTag::DMS, "[NAPI]Failed to convert parameter to scale[%lu]", i + 1);
            napi_throw(env, CreateJsError(env, static_cast<int32_t>(WSErrorCode::WS_ERROR_INVALID_PARAM),
                "Input parameter is missing or invalid"));
            return NapiGetUndefined(env);
        }
