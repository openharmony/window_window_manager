...
NativeValue* JsSceneSession::Create(NativeEngine& engine, const sptr<SceneSession>& session)
{
    ...
    object->SetProperty("persistentId", CreateJsValue(engine, static_cast<uint32_t>(session->GetPersistentId())));
    object->SetProperty("parentId", CreateJsValue(engine, static_cast<uint32_t>(session->GetParentPersistentId())));
