...
{
    if (g_handle != nullptr) {
        TLOGW(WmsLogTag::DMS_SSM, "motion plugin has already exits.");
        return true;
    }
    int32_t cnt = 0;
    ...
    do {
        ...
        TLOGI(WmsLogTag::DMS_SSM, "dlopen %{public}s, retry cnt: %{public}d", PLUGIN_SO_PATH.c_str(), cnt);
...
{
    TLOGI(WmsLogTag::DMS_SSM, "unload motion plugin.");
    if (g_handle != nullptr) {
        dlclose(g_handle);
        g_handle = nullptr;
    }
...
{
    if (callback == nullptr) {
        TLOGE(WmsLogTag::DMS_SSM, "callback is nullptr");
        return false;
    }
    if (g_handle == nullptr) {
        TLOGE(WmsLogTag::DMS_SSM, "g_handle is nullptr");
        return false;
    }
    MotionSubscribeCallbackPtr func = (MotionSubscribeCallbackPtr)(dlsym(g_handle, "MotionSubscribeCallback"));
    const char* dlsymError = dlerror();
    if  (dlsymError) {
        TLOGE(WmsLogTag::DMS_SSM, "dlsym error: %{public}s", dlsymError);
        return false;
    }
