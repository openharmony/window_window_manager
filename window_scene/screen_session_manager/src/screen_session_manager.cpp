...
void ScreenSessionManager::OnVirtualScreenChange(ScreenId screenId, ScreenEvent screenEvent)
{
    ...
    if (screenEvent == ScreenEvent::DISCONNECTED) {
        std::map<int32_t, sptr<IScreenSessionManagerClient>> clientProxyMap;
        {
            std::lock_guard<std::mutex> lock(clientProxyMutex_);
            clientProxyMap = clientProxyMap_;
        }
        for (const auto& [userId, clientProxy] : clientProxyMap) {
            if (clientProxy) {
                clientProxy->OnScreenConnectionChanged(GetSessionOption(screenSession, screenId),
...
void ScreenSessionManager::DisConnectScreenIfScreenInfoNull(sptr<ScreenSession>& screenSession)
{
   ...
   if (screenSession == nullptr || clientProxy_ == nullptr) {
        TLOGE(WmsLogTag::DMS, "screenSession or clientProxy_ is null");
        return;
    }
    if (IsDefaultMirrMode(screenSession->GetRSScreenId())) {
        clientProxy_->OnScreenConnectionChanged(
...
void ScreenSessionManager::ScbStatusRecoveryWhenSwitchUser(std::vector<int32_t> oldScbPids, int32_t newScbPid)
{
    ...
    if (g_foldScreenFlag && oldScbDisplayMode_ != GetFoldDisplayMode() &&
        ...
        if (FoldScreenStateInternel::IsSecondaryDisplayFoldDevice()) {
            ...
            OnVerticalChangeBoundsWhenSwitchUser(screenSession);
            screenSession->PropertyChange(screenSession->GetScreenProperty(),
