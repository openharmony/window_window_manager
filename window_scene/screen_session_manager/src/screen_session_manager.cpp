    ...
    const sptr<ScreenSession>& screenSession, int32_t uid)
{
    ...
    if (displayHookMap_.find(uid) != displayHookMap_.end() && !displayHookMap_[uid].isFullScreenInForceSplit_) {
        ...
        oss << "hW: " << info.width_ << ", hH: " << info.height_ << ", hD: " << info.density_
            ...
            << ", dW: " << displayInfo->GetWidth() << ", dH: " << displayInfo->GetHeight()
            << ", dR: " << static_cast<uint32_t>(displayInfo->GetRotation())
            << ", dO: " << static_cast<uint32_t>(displayInfo->GetDisplayOrientation());
        std::regex pattern("%");
        TLOGNFI(WmsLogTag::DMS, "%{private}s", std::regex_replace(oss.str(), pattern, "%%").c_str());
        ...
        if (info.enableHookDisplayOrientation_ && IsValidDisplayOrientation(info.displayOrientation_)) {
                                                   ...
                                                   const sptr<IRemoteObject>& displayManagerAgent)
{
    ...
    TLOGNFI(WmsLogTag::DMS, "missionID size:%{public}ud", static_cast<uint32_t>(option.missionIds_.size()));
    ...
    DmErrorCode* errorCode)
{
    ...
    if ((Permission::IsSystemCalling() && (Permission::CheckCallingPermission(SCREEN_CAPTURE_PERMISSION) ||
        ...
        Drawing::Rect rect = { static_cast<float>(orgRect.posX_), static_cast<float>(orgRect.posY_),
            static_cast<float>(orgRect.posX_ + orgRect.width_), static_cast<float>(orgRect.posY_ + orgRect.height_) };
...
DMError ScreenSessionManager::HasPrivateWindow(DisplayId id, bool& hasPrivateWindow)
{
    ...
    TLOGNFI(WmsLogTag::DMS, "id: %{public}" PRIu64" privW: %{public}u",
...
{
    TLOGD(WmsLogTag::DMS, "screenId: %{public}" PRIu64 " sensorRotation: %{public}f", screenId, sensorRotation);
    auto clientProxy = GetClientProxy();
    if (!clientProxy) {
        TLOGNFI(WmsLogTag::DMS, "clientProxy_ is null");
        return;
