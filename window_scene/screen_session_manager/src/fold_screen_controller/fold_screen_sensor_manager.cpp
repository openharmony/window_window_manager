...
void FoldScreenSensorManager::RegisterPostureCallback()
{
    ...
    if (subscribeRet != SENSOR_SUCCESS || setBatchRet != SENSOR_SUCCESS || activateRet != SENSOR_SUCCESS) {
        TLOGE(WmsLogTag::DMS_SSM, "RegisterPostureCallback failed.");
    } else {
        registerPosture_ = true;
        TLOGI(WmsLogTag::DMS_SSM, "FoldScreenSensorManager.RegisterPostureCallback success.");
    }
}
...
void FoldScreenSensorManager::UnRegisterPostureCallback()
{
    ...
    TLOGI(WmsLogTag::DMS_SSM, "UnRegisterPostureCallback, deactivateRet: %{public}d, unsubscribeRet: %{public}d",
...
void FoldScreenSensorManager::RegisterHallCallback()
{
    ...
    TLOGI(WmsLogTag::DMS_SSM, "RegisterHallCallback, subscribeRet: %{public}d", subscribeRet);
    int32_t setBatchRet = SetBatch(SENSOR_TYPE_ID_HALL_EXT, &hallUser, POSTURE_INTERVAL, POSTURE_INTERVAL);
    TLOGI(WmsLogTag::DMS_SSM, "RegisterHallCallback, setBatchRet: %{public}d", setBatchRet);
    int32_t activateRet = ActivateSensor(SENSOR_TYPE_ID_HALL_EXT, &hallUser);
    TLOGI(WmsLogTag::DMS_SSM, "RegisterHallCallback, activateRet: %{public}d", activateRet);
    if (subscribeRet != SENSOR_SUCCESS || setBatchRet != SENSOR_SUCCESS || activateRet != SENSOR_SUCCESS) {
        TLOGE(WmsLogTag::DMS_SSM, "RegisterHallCallback failed.");
    }
}
...
void FoldScreenSensorManager::UnRegisterHallCallback()
{
    ...
    if (deactivateRet1 == SENSOR_SUCCESS && unsubscribeRet1 == SENSOR_SUCCESS) {
        TLOGI(WmsLogTag::DMS_SSM, "FoldScreenSensorManager.UnRegisterHallCallback success.");
    }
}
...
void FoldScreenSensorManager::HandlePostureData(const SensorEvent * const event)
{
    ...
    if (globalHall == USHRT_MAX || std::isless(globalAngle, ANGLE_MIN_VAL) ||
        ...
        TLOGE(WmsLogTag::DMS_SSM, "Invalid value, hall value is: %{public}u, angle value is: %{public}f.",
            globalHall, globalAngle);
        return;
    }
    TLOGD(WmsLogTag::DMS_SSM, "angle value in PostureData is: %{public}f.", globalAngle);
...
void FoldScreenSensorManager::HandleHallData(const SensorEvent * const event)
{
    ...
    if (event[SENSOR_EVENT_FIRST_DATA].dataLen < sizeof(ExtHallData)) {
        TLOGI(WmsLogTag::DMS_SSM, "SensorEvent dataLen less than hall data size.");
        return;
    }
    ...
    if (!(flag & (1 << 1))) {
        TLOGI(WmsLogTag::DMS_SSM, "NOT Support Extend Hall.");
        return;
    }
    if (globalHall == static_cast<uint16_t>((*extHallData).hall)) {
        TLOGI(WmsLogTag::DMS_SSM, "Hall don't change, hall = %{public}u", globalHall);
        return;
    }
    ...
    TLOGI(WmsLogTag::DMS_SSM, "hall value is: %{public}u, angle value is: %{public}f", globalHall, globalAngle);
    if (!registerPosture_) {
...
{
    TLOGI(WmsLogTag::DMS_SSM, "TriggerDisplaySwitch hall value is: %{public}u, angle value is: %{public}f",
        globalHall, globalAngle);
    if (!registerPosture_) {
        globalAngle = ANGLE_MIN_VAL;
    } else {
