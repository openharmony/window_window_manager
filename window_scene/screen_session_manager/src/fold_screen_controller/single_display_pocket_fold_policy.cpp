...
{
    TLOGI(WmsLogTag::DMS_SSM, "SingleDisplayPocketFoldPolicy created");
...
void SingleDisplayPocketFoldPolicy::SetdisplayModeChangeStatus(bool status, bool isOnBootAnimation)
{
    if (status) {
        ...
        TLOGI(WmsLogTag::DMS_SSM, "displaymodechange start, taskCount: %{public}d", pengdingTask_.load());
    } else {
        pengdingTask_ --;
        if (pengdingTask_ != 0) {
            TLOGI(WmsLogTag::DMS_SSM, "displaymodechange 1 task finished, %{public}d task left", pengdingTask_.load());
            return;
        }
        displayModeChangeRunning_ = false;
        endTimePoint_ = std::chrono::steady_clock::now();
        TLOGI(WmsLogTag::DMS_SSM, "displaymodechange finished, current displaymode: %{public}ud,"
    ...
    DisplayModeChangeReason reason)
{
    ...
    if (GetModeChangeRunningStatus()) {
        TLOGW(WmsLogTag::DMS_SSM, "last process not complete, skip mode: %{public}d", displayMode);
        return;
    }
    TLOGI(WmsLogTag::DMS_SSM, "start change displaymode: %{public}d, lastElapsedMs: %{public}" PRId64 "ms",
    ...
    if (screenSession == nullptr) {
        TLOGE(WmsLogTag::DMS_SSM, "default screenSession is null");
        return;
    }
    HITRACE_METER_FMT(HITRACE_TAG_WINDOW_MANAGER, "ssm:ChangeScreenDisplayMode(displayMode = %" PRIu64")", displayMode);
    {
        ...
        if (currentDisplayMode_ == displayMode) {
            TLOGW(WmsLogTag::DMS_SSM, "ChangeScreenDisplayMode already in displayMode %{public}d", displayMode);
            return;
        }
    }
    ...
    FoldDisplayMode displayMode, DisplayModeChangeReason reason)
{
    switch (displayMode) {
        ...
        case FoldDisplayMode::UNKNOWN: {
            TLOGI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayMode displayMode is unknown");
            break;
        }
        default: {
            TLOGI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayMode displayMode is invalid");
            break;
        }
    }
}
...
{
    TLOGI(WmsLogTag::DMS_SSM, "SendSensorResult FoldStatus: %{public}d", foldStatus);
...
{
    TLOGI(WmsLogTag::DMS_SSM, "GetCurrentFoldCreaseRegion");
    return currentFoldCreaseRegion_;
}

void SingleDisplayPocketFoldPolicy::LockDisplayStatus(bool locked)
{
    TLOGI(WmsLogTag::DMS_SSM, "LockDisplayStatus locked: %{public}d", locked);
    lockDisplayStatus_ = locked;
}

void SingleDisplayPocketFoldPolicy::SetOnBootAnimation(bool onBootAnimation)
{
    TLOGI(WmsLogTag::DMS_SSM, "SetOnBootAnimation onBootAnimation: %{public}d", onBootAnimation);
    onBootAnimation_ = onBootAnimation;
    if (!onBootAnimation_) {
        TLOGI(WmsLogTag::DMS_SSM, "SetOnBootAnimation when boot animation finished, change display mode");
...
{
    TLOGI(WmsLogTag::DMS_SSM, "CurrentScreen(%{public}" PRIu64 ")", screenId_);
    FoldDisplayMode displayMode = GetModeMatchStatus();
    if (currentDisplayMode_ != displayMode) {
        ChangeScreenDisplayMode(displayMode);
    }
...
FoldDisplayMode SingleDisplayPocketFoldPolicy::GetModeMatchStatus()
{
    ...
    switch (currentFoldStatus_) {
        ...
        default: {
            TLOGI(WmsLogTag::DMS_SSM, "GetModeMatchStatus FoldStatus is invalid");
        }
    }
    return displayMode;
}
...
void SingleDisplayPocketFoldPolicy::ReportFoldDisplayModeChange(FoldDisplayMode displayMode)
{
    ...
    TLOGI(WmsLogTag::DMS_SSM, "ReportFoldDisplayModeChange displayMode: %{public}d", mode);
    ...
    if (ret != 0) {
        TLOGE(WmsLogTag::DMS_SSM, "ReportFoldDisplayModeChange Write HiSysEvent error, ret: %{public}d", ret);
    }
}

void SingleDisplayPocketFoldPolicy::ReportFoldStatusChangeBegin(int32_t offScreen, int32_t onScreen)
{
    TLOGI(WmsLogTag::DMS_SSM, "ReportFoldStatusChangeBegin offScreen: %{public}d, onScreen: %{public}d",
        offScreen, onScreen);
    int32_t ret = HiSysEventWrite(
        OHOS::HiviewDFX::HiSysEvent::Domain::WINDOW_MANAGER,
    ...
    if (ret != 0) {
        TLOGE(WmsLogTag::DMS_SSM, "ReportFoldStatusChangeBegin Write HiSysEvent error, ret: %{public}d", ret);
    }
}

void SingleDisplayPocketFoldPolicy::ChangeScreenDisplayModeToMainWhenFoldScreenOn(sptr<ScreenSession> screenSession)
{
    TLOGI(WmsLogTag::DMS_SSM, "IsFoldScreenOn is true, begin.");
    ReportFoldStatusChangeBegin(static_cast<int32_t>(SCREEN_ID_FULL),
        static_cast<int32_t>(SCREEN_ID_MAIN));
    auto taskScreenOnMain = [=] {
        // off full screen
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToMain: IsFoldScreenOn is true, screenIdFull OFF.");
        ...
        // on main screen
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToMain: IsFoldScreenOn is true, screenIdMain ON.");
...
{
    TLOGI(WmsLogTag::DMS_SSM, "IsFoldScreenOn is false, begin.");
    // off full screen
    auto taskScreenOffMainOff = [=] {
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToMain: IsFoldScreenOn is false, screenIdFull OFF.");
        screenId_ = SCREEN_ID_FULL;
        ChangeScreenDisplayModePower(SCREEN_ID_FULL, ScreenPowerStatus::POWER_STATUS_OFF);
        SetdisplayModeChangeStatus(false);
    };
    ...
    auto taskScreenOnMainChangeScreenId = [=] {
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToMain: IsFoldScreenOn is false, Change ScreenId to Main.");
        screenId_ = SCREEN_ID_MAIN;
#ifdef TP_FEATURE_ENABLE
...
void SingleDisplayPocketFoldPolicy::ChangeScreenDisplayModeToFullWhenFoldScreenOn(sptr<ScreenSession> screenSession)
{
    ...
    auto taskScreenOnFull = [=] {
        // off main screen
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToFull: IsFoldScreenOn is true, screenIdMain OFF.");
        screenId_ = SCREEN_ID_MAIN;
        ChangeScreenDisplayModePower(SCREEN_ID_MAIN, ScreenPowerStatus::POWER_STATUS_OFF);
        SetdisplayModeChangeStatus(false);

        // on full screen
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToFull: IsFoldScreenOn is true, screenIdFull ON.");
        screenId_ = SCREEN_ID_FULL;
    ...
    DisplayModeChangeReason reason)
{
    ...
    auto taskScreenOffFullOff = [=] {
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToFull: IsFoldScreenOn is false, screenIdMain OFF.");
        screenId_ = SCREEN_ID_MAIN;
        ChangeScreenDisplayModePower(SCREEN_ID_MAIN, ScreenPowerStatus::POWER_STATUS_OFF);
        SetdisplayModeChangeStatus(false);
    };
    ...
    auto taskScreenOnFullOn = [=] {
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToFull: IsFoldScreenOn is false, screenIdFull ON.");
        screenId_ = SCREEN_ID_FULL;
    ...
    ScreenPropertyChangeReason reason)
{
    ...
    TLOGI(WmsLogTag::DMS_SSM, "screenBounds : width_= %{public}f, height_= %{public}f",
...
{
    TLOGI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToMainOnBootAnimation");
...
{
    TLOGI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToFullOnBootAnimation");
...
void SingleDisplayPocketFoldPolicy::BootAnimationFinishPowerInit()
{
    ...
    if (RSInterfaces::GetInstance().GetActiveScreenId() == SCREEN_ID_FULL) {
        // 同显切内屏：外屏下电
        TLOGI(WmsLogTag::DMS_SSM, "Fold Screen Power main screen off.");
        ...
        // 同显切外屏：双屏都灭再外屏上电
        TLOGI(WmsLogTag::DMS_SSM, "Fold Screen Power all screen off.");
        RSInterfaces::GetInstance().SetScreenPowerStatus(SCREEN_ID_MAIN, ScreenPowerStatus::POWER_STATUS_OFF);
        ...
        TLOGI(WmsLogTag::DMS_SSM, "Fold Screen Power main screen on.");
        RSInterfaces::GetInstance().SetScreenPowerStatus(SCREEN_ID_MAIN, ScreenPowerStatus::POWER_STATUS_ON);
    } else {
        TLOGI(WmsLogTag::DMS_SSM, "Fold Screen Power Init, invalid active screen id");
    }
}

void SingleDisplayPocketFoldPolicy::ChangeOnTentMode(FoldStatus currentState)
{
    TLOGI(WmsLogTag::DMS_SSM, "Enter tent mode, current state:%{public}d, change display mode to MAIN", currentState);
    if (currentState == FoldStatus::EXPAND || currentState == FoldStatus::HALF_FOLD) {
        ...
        TLOGE(WmsLogTag::DMS_SSM, "current state:%{public}d invalid", currentState);
    }
}
...
{
    TLOGI(WmsLogTag::DMS_SSM, "screenId: %{public}" PRIu64 ", command: %{public}d",
        screenId, command);
    ...
    if (transactionProxy != nullptr) {
        TLOGI(WmsLogTag::DMS_SSM, "add or remove displayNode");
...
void SingleDisplayPocketFoldPolicy::ChangeScreenDisplayModeToCoordination()
{
    ...
    if (ScreenSessionManager::GetInstance().GetCoordinationFlag()) {
        TLOGW(WmsLogTag::DMS_SSM, "change displaymode to coordination skipped, current coordination flag is true");
        return;
    }
    TLOGI(WmsLogTag::DMS_SSM, "change displaymode to coordination current mode=%{public}d", currentDisplayMode_);
    ...
    auto taskScreenOnMainOn = [=] {
        TLOGNI(WmsLogTag::DMS_SSM, "ChangeScreenDisplayModeToCoordination: screenIdMain ON.");
        NotifyRefreshRateEvent(true);
        ChangeScreenDisplayModePower(SCREEN_ID_MAIN, ScreenPowerStatus::POWER_STATUS_ON);
...
void SingleDisplayPocketFoldPolicy::CloseCoordinationScreen()
{
    std::lock_guard<std::mutex> lock(coordinationMutex_);
    if (!ScreenSessionManager::GetInstance().GetCoordinationFlag()) {
        TLOGW(WmsLogTag::DMS_SSM, "change displaymode to coordination skipped, current coordination flag is false");
        return;
    }
    TLOGI(WmsLogTag::DMS_SSM, "Close Coordination Screen current mode=%{public}d", currentDisplayMode_);
    
    // on main screen
    auto taskScreenOnMainOFF = [=] {
        TLOGNI(WmsLogTag::DMS_SSM, "CloseCoordinationScreen: screenIdMain OFF.");
        ChangeScreenDisplayModePower(SCREEN_ID_MAIN, ScreenPowerStatus::POWER_STATUS_OFF);
        NotifyRefreshRateEvent(false);
    };
...
void SingleDisplayPocketFoldPolicy::ExitCoordination()
{
    ...
    currentDisplayMode_ = displayMode;
    lastDisplayMode_ = displayMode;
    TLOGI(WmsLogTag::DMS_SSM, "Exit coordination, current display mode:%{public}d", displayMode);
    ScreenSessionManager::GetInstance().NotifyDisplayModeChanged(displayMode);
}
