...
{
    TLOGI(WmsLogTag::DMS_DMSERVER, "Set isWaterfallDisplay: %{public}u", isWaterfallDisplay);
...
void DisplayCutoutController::SetCurvedScreenBoundary(std::vector<int> curvedScreenBoundary)
{
    ...
    TLOGI(WmsLogTag::DMS_DMSERVER, "Set curvedScreenBoundary");
...
{
    TLOGI(WmsLogTag::DMS_DMSERVER, "Set SvgPath: %{public}s", svgPath.c_str());
...
{
    TLOGD(WmsLogTag::DMS_DMSERVER, "Get Cutout Info");
...
void DisplayCutoutController::CheckBoundingRectsBoundary(DisplayId displayId, std::vector<DMRect>& boundingRects)
{
    ...
    if (modes == nullptr) {
        TLOGE(WmsLogTag::DMS_DMSERVER, "DisplayId is invalid");
        return;
    }
    ...
    for (auto iter = boundingRects.begin(); iter != boundingRects.end();) {
        ...
        if (boundingRect.posX_ < 0 || boundingRect.posY_ < 0 ||
            ...
            TLOGE(WmsLogTag::DMS_DMSERVER, "boundingRect boundary is invalid");
...
DMRect DisplayCutoutController::CalcCutoutBoundingRect(std::string svgPath)
{
    ...
    if (!SkParsePath::FromSVGString(svgPath.c_str(), &skCutoutSvgPath)) {
        TLOGE(WmsLogTag::DMS_DMSERVER, "Parse svg string path failed.");
        return emptyRect;
    }
    SkRect skRect = skCutoutSvgPath.computeTightBounds();
    if (skRect.isEmpty()) {
        TLOGW(WmsLogTag::DMS_DMSERVER, "Get empty skRect");
        return emptyRect;
    }
    SkIRect skiRect = skRect.roundOut();
    if (skiRect.isEmpty()) {
        TLOGW(WmsLogTag::DMS_DMSERVER, "Get empty skiRect");
        return emptyRect;
    }
    ...
    TLOGI(WmsLogTag::DMS_DMSERVER, "calc rect from path,[%{public}d, %{public}d, %{public}u, %{public}u]",
...
void DisplayCutoutController::CalcBuiltInDisplayWaterfallRects()
{
    ...
    if (!isWaterfallDisplay_) {
        TLOGI(WmsLogTag::DMS_DMSERVER, "not waterfall display");
        waterfallDisplayAreaRects_ = emptyRects;
        return;
    }
    if (curvedScreenBoundary_.empty()) {
        TLOGI(WmsLogTag::DMS_DMSERVER, "curved screen boundary is empty");
        waterfallDisplayAreaRects_ = emptyRects;
        return;
    }
    ...
    if (!modes) {
        TLOGE(WmsLogTag::DMS_DMSERVER, "support screen modes get failed");
        waterfallDisplayAreaRects_ = emptyRects;
        return;
    }
    uint32_t displayHeight = modes->height_;
    ...
    if ((left > displayWidth / 2) || (right > displayWidth / 2) || // invalid if more than 1/2 width
        ...
        TLOGE(WmsLogTag::DMS_DMSERVER, "Curved screen boundary data is not valid.");
        waterfallDisplayAreaRects_ = emptyRects;
        return;
    }
...
void DisplayCutoutController::TransferBoundingRectsByRotation(DisplayId displayId, std::vector<DMRect>& boundingRects)
{
    ...
    if (!displayInfo) {
        TLOGE(WmsLogTag::DMS_DMSERVER, "display invaild");
        return;
    }
...
uint32_t DisplayCutoutController::GetWaterfallAreaCompressionSizeWhenHorizontal()
{
    if (!isWaterfallDisplay_ || !isWaterfallAreaCompressionEnableWhenHorizontal_) {
        TLOGW(WmsLogTag::DMS_DMSERVER, "Not waterfall display or not enable waterfall compression");
