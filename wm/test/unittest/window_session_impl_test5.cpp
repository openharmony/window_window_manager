...
HWTEST_F(WindowSessionImplTest5, SwitchSubWindow, Function | SmallTest | Level1)
{
    ...
    subWindow->property_->SetPersistentId(SUB_WINDOW_PERSISTENT_ID_ONE);
    subWindow->property_->SetDecorEnable(true);
    subWindow->property_->SetWindowMode(WindowMode::WINDOW_MODE_FLOATING);
    subWindow->windowSystemConfig_.uiType_ = "pad";
    ...
    // cover emprty map
    subWindow->SwitchSubWindow(PERSISTENT_ID);

    std::vector<sptr<WindowSessionImpl>> vec;
    WindowSessionImpl::subWindowSessionMap_.insert(std::pair<int32_t,
        std::vector<sptr<WindowSessionImpl>>>(PERSISTENT_ID, vec));
    WindowSessionImpl::subWindowSessionMap_[PERSISTENT_ID].push_back(subWindow);
