...
WMError WindowSceneSessionImpl::AdjustKeyboardLayout(const KeyboardLayoutParams params)
{
    TLOGI(WmsLogTag::WMS_KEYBOARD, "gravity: %{public}u, "
        "land: avoid=%{public}d, kbd=%{public}s, panel=%{public}s, "
        "port: avoid=%{public}d, kbd=%{public}s, panel=%{public}s",
        static_cast<uint32_t>(params.gravity_),
        params.landscapeAvoidHeight_,
        params.LandscapeKeyboardRect_.ToString().c_str(),
        params.LandscapePanelRect_.ToString().c_str(),
        params.portraitAvoidHeight_,
