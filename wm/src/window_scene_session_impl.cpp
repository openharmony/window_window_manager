...
void WindowSceneSessionImpl::UpdateWindowState()
{
    ...
    if (WindowHelper::IsMainWindow(windowType)) {
        ...
        if ((isSubWindow || isDialogWindow || isSystemWindow) && property_->GetDragEnabled()) {
...
WMError WindowSceneSessionImpl::SetLayoutFullScreen(bool status)
{
    ...
    if ((windowSystemConfig_.uiType_ == UI_TYPE_PC || IsFreeMultiWindowMode()) && property_->GetCompatibleModeInPc()) {
    ...
    if (WindowHelper::IsMainWindow(GetType()) &&
        ...
        windowSystemConfig_.uiType_ != UI_TYPE_PAD) {
...
WMError WindowSceneSessionImpl::Restore()
{
    ...
    if (!(windowSystemConfig_.uiType_ == UI_TYPE_PC || property_->GetIsPcAppInPad())) {
