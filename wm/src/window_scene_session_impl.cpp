...
bool WindowSceneSessionImpl::IsFullScreenSizeWindow(uint32_t width, uint32_t height)
{
    ...
    // if is super fold device and in half fold state, check c side screen
    auto display_c = SingletonContainer::Get<DisplayManager>().GetDisplayById(DISPLAY_ID_C);
    if (display_c == nullptr) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "display c is null");
        return false;
    }
    auto displayInfo_c = display_c->GetDisplayInfo();
    if (displayInfo_c == nullptr) {
        TLOGE(WmsLogTag::WMS_LAYOUT, "displayInfo c is null");
        return false;
    }
    displayWidth = static_cast<uint32_t>(displayInfo_c->GetWidth());
    displayHeight = static_cast<uint32_t>(displayInfo_c->GetHeight());
    if (displayWidth == width && displayHeight == height) {
        return true;
    }
    return false;
    ...
    uint32_t displayHeight, float vpr)
{
    ...
    if (WindowHelper::IsMainWindow(GetType())) {
        ...
        systemLimits.minWidth_ = UpdateConfigVal(0, displayWidth, windowSystemConfig_.miniWidthOfDialogWindow_,
                                                 MIN_FLOATING_WIDTH, vpr);
        systemLimits.minHeight_ = UpdateConfigVal(0, displayHeight, windowSystemConfig_.miniHeightOfDialogWindow_,
                                                  MIN_FLOATING_HEIGHT, vpr);
    } else if (WindowHelper::IsSystemWindow(GetType())) {
...
bool WindowSceneSessionImpl::IsDecorEnable() const
{
    ...
    if (isCompatibleModeInPc && GetWindowMode() == WindowMode::WINDOW_MODE_FULLSCREEN && !isLayoutFullScreen) {
        enable = false;
    }
...
{
    if (auto uiContent = GetUIContentSharedPtr()) {
        TLOGI(WmsLogTag::WMS_ATTRIBUTE, "ace winId: %{public}u", GetWindowId());
...
{
    if (IsWindowSessionInvalid()) {
        TLOGE(WmsLogTag::WMS_COMPAT, "window invalid!");
        return WSError::WS_ERROR_INVALID_WINDOW;
    }
    if (!property_->GetCompatibleModeInPc()) {
        TLOGE(WmsLogTag::WMS_COMPAT, "is not CompatibleModeInPc, can not Recover");
        return WSError::WS_ERROR_INVALID_WINDOW;
    }
    Recover();
    return WSError::WS_OK;
...
WSError WindowSceneSessionImpl::CompatibleFullScreenMinimize()
{
    ...
    if (!property_->GetCompatibleModeInPc()) {
        TLOGE(WmsLogTag::WMS_COMPAT, "is not CompatibleModeInPc, can not Minimize");
        return WSError::WS_ERROR_INVALID_WINDOW;
    }
    Minimize();
    return WSError::WS_OK;
...
WSError WindowSceneSessionImpl::CompatibleFullScreenClose()
{
    ...
    if (!property_->GetCompatibleModeInPc()) {
        TLOGE(WmsLogTag::WMS_COMPAT, "is not CompatibleModeInPc, can not Close");
        return WSError::WS_ERROR_INVALID_WINDOW;
    }
    Close();
    return WSError::WS_OK;
...
void WindowSceneSessionImpl::fillWindowLimits(WindowLimits& windowLimits)
{
    ...
    WLOGFI("success! Id:%{public}u, minWidth:%{public}u, minHeight:%{public}u, "
