...
sptr<Window> StaticCall::CreateWindow(sptr<WindowOption>& option, std::shared_ptr<AbilityRuntime::Context> context,
    const sptr<IRemoteObject>& iSession, const std::string& identityToken, bool isAbilityHookEnd)
{
    WMError errCode = WMError::WM_OK;
    return Window::Create(option, context, iSession, errCode, identityToken, isAbilityHookEnd);
}
