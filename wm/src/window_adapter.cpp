    ...
    const sptr<IRemoteObject>& token, uint64_t surfaceNodeId, bool isConstrained)
{
    ...
    auto wmsProxy = GetWindowManagerServiceProxy();
    CHECK_PROXY_RETURN_IF_NULL(wmsProxy);
    wmsProxy->AddExtensionWindowStageToSCB(sessionStage, token, surfaceNodeId, isConstrained);
}
    ...
    const sptr<IRemoteObject>& token, bool isConstrained)
{
    ...
    auto wmsProxy = GetWindowManagerServiceProxy();
    CHECK_PROXY_RETURN_IF_NULL(wmsProxy);
    wmsProxy->RemoveExtensionWindowStageFromSCB(sessionStage, token, isConstrained);
}
