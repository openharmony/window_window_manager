...
WMError WindowSessionImpl::Show(uint32_t reason, bool withAnimation, bool withFocus, bool needAttach)
{
...
WMError WindowSessionImpl::Hide(uint32_t reason, bool withAnimation, bool isFromInnerkits, bool needDetach)
{
...
void WindowSessionImpl::NotifyPointerEvent(const std::shared_ptr<MMI::PointerEvent>& pointerEvent)
{
    ...
    if (auto uiContent = GetUIContentSharedPtr()) {
        ...
        if (pointerEvent->GetPointerAction() != MMI::PointerEvent::POINTER_ACTION_MOVE) {
            TLOGW(WmsLogTag::WMS_INPUT_KEY_FLOW, "not consume, wid:%{public}u", GetWindowId());
        }
        pointerEvent->MarkProcessed();
    }
}
