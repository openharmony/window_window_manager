...
WindowSessionImpl::WindowSessionImpl(const sptr<WindowOption>& option)
{
    ...
    property_->SetIsConstrained(option->GetIsConstrained());
...
RSSurfaceNode::SharedPtr WindowSessionImpl::CreateSurfaceNode(std::string name, WindowType type)
{
    ...
    return RSSurfaceNode::Create(rsSurfaceNodeConfig, rsSurfaceNodeType, true, property_->GetIsConstrained());
}
...
void WindowSessionImpl::DestroySubWindow()
{
    ...
    if (subIter != subWindowSessionMap_.end()) {
        ...
        if (property_->GetExtensionFlag() && subWindowSessionMap_.empty()) {
            auto extensionWindow = FindExtensionWindowWithContext();
...
{
    bool isPC = windowSystemConfig_.uiType_ == UI_TYPE_PC;
...
void WindowSessionImpl::NotifyPointerEvent(const std::shared_ptr<MMI::PointerEvent>& pointerEvent)
{
    ...
    if (uiContent != nullptr) {
        ...
        TLOGW(WmsLogTag::WMS_INPUT_KEY_FLOW, "pointerEvent is not consumed, windowId: %{public}u", GetWindowId());
        pointerEvent->MarkProcessed();
    }
}
