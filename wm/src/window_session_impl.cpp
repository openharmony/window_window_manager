...
diff --git a/wm/include/window_session_impl.h b/wm/include/window_session_impl.h
index 2bbe4f5271532843d4176bdd6924ae602ede87c2..b976e71a04031edc1bb2c5cdde8bffa005ba5409 100644
--- a/wm/include/window_session_impl.h
+++ b/wm/include/window_session_impl.h
...
diff --git a/wm/src/window_extension_session_impl.cpp b/wm/src/window_extension_session_impl.cpp
index d75783b9d8d7f98206e6f1444db01d024b604a2a..2a7d688ddf9d16a2b0af0ebbfb9ffe66bea35c05 100644
--- a/wm/src/window_extension_session_impl.cpp
+++ b/wm/src/window_extension_session_impl.cpp
...
+    do{                                                                        \
+        if ((hostSession) == nullptr) {                                        \
+            TLOGE(WmsLog::DEFAULT, "hostSession is null");                     \
+            return;                                                            \
+        }                                                                      \
+    } while (false)
+
+#define CHECK_HOST_SESSION_RETURN_ERROR_IF_NULL(hostSession)                   \
+    do{                                                                        \
+        if ((hostSession) == nullptr) {                                        \
+            TLOGE(WmsLog::DEFAULT, "hostSession is null");                     \
+            return;                                                            \
...
+    {
+        std::lock_guard<std::mute> lock(hostSessionMutex_);
+        hostSession_ = iSession;
+    }
     context_ = context;
     WMError ret = Connect();
     if (ret == WMError::WM_OK) {
@@ -107,9 +126,13 @@ WMError WindowExtensionSessionImpl::Destroy(bool needNotifyServer, bool needClea
         return WMError::WM_ERROR_INVALID_WINDOW;
     }
...
+        }
     ...
     NotifyBeforeDestroy(GetWindowName());
     {
@@ -117,8 +140,11 @@ WMError WindowExtensionSessionImpl::Destroy(bool needNotifyServer, bool needClea
         state_ = WindowState::STATE_DESTROYED;
         requestState_ = WindowState::STATE_DESTROYED;
     }
...
@@ -170,7 +196,9 @@ WMError WindowExtensionSessionImpl::TransferAbilityResult(uint32_t resultCode, c
         WLOGFE("Window session invalid.");
         return WMError::WM_ERROR_REPEAT_OPERATION;
     }
-    return static_cast<WMError>(hostSession_->TransferAbilityResult(resultCode, want));
+    auto hostSession = GetHostSession();
+    CHECK_HOST_SESSION_RETURN_ERROR_IF_NULL(hostSession, WM_ERROR_INVALID_WINDOW);
...
@@ -179,6 +207,8 @@ WMError WindowExtensionSessionImpl::TransferExtensionData(const AAFwk::WantParam
         WLOGFE("Window session invalid.");
         return WMError::WM_ERROR_REPEAT_OPERATION;
     }
+    auto hostSession = GetHostSession();
...
@@ -189,7 +219,9 @@ void WindowExtensionSessionImpl::RegisterTransferComponentDataListener(const Not
         return;
     }
...
@@ -220,17 +252,17 @@ void WindowExtensionSessionImpl::RegisterTransferComponentDataForResultListener(
         return;
     }
...
@@ -587,7 +619,9 @@ WMError WindowExtensionSessionImpl::TransferAccessibilityEvent(const Accessibili
         WLOGFE("Window session invalid.");
         return WMError::WM_ERROR_INVALID_WINDOW;
     }
...
@@ -624,10 +658,8 @@ WMError WindowExtensionSessionImpl::UnregisterOccupiedAreaChangeListener(
...
@@ -675,7 +707,8 @@ WMError WindowExtensionSessionImpl::Hide(uint32_t reason, bool withAnimation, bo
         NotifyBackgroundFailed(WMError::WM_DO_NOTHING);
         return WMError::WM_OK;
     }
...
index 80fd41810c81f78c70077d716fe3e1267137ccb6..1eb32098b86440d311ea82481e5da39770e32301 100644
--- a/wm/src/window_scene_session_impl.cpp
+++ b/wm/src/window_scene_session_impl.cpp
@@ -68,6 +68,23 @@ union WSColorParam {
 #endif
     uint32_t value;
 };
+
...
@@ -233,7 +250,10 @@ WMError WindowSceneSessionImpl::CreateAndConnectSpecificSession()
...
@@ -312,7 +332,10 @@ WMError WindowSceneSessionImpl::RecoverAndConnectSpecificSession()
...
@@ -355,6 +378,7 @@ WMError WindowSceneSessionImpl::RecoverAndReconnectSceneSession()
     }
...
@@ -412,7 +436,10 @@ WMError WindowSceneSessionImpl::Create(const std::shared_ptr<AbilityRuntime::Con
         return ret;
     }
     SetDefaultDisplayIdIfNeed();
-    hostSession_ = iSession;
...
@@ -425,7 +452,7 @@ WMError WindowSceneSessionImpl::Create(const std::shared_ptr<AbilityRuntime::Con
     }
...
@@ -475,16 +502,18 @@ bool WindowSceneSessionImpl::HandlePointDownEvent(const std::shared_ptr<MMI::Poi
...
@@ -519,7 +548,10 @@ void WindowSceneSessionImpl::ConsumePointerEventInner(const std::shared_ptr<MMI:
...
+        }
     ...
     if (needNotifyEvent) {
@@ -546,7 +578,7 @@ void WindowSceneSessionImpl::ConsumePointerEvent(const std::shared_ptr<MMI::Poin
         return;
     }
...
+    if (GetHostSession() == nullptr) {
         ...
         pointerEvent->MarkProcessed();
         return;
@@ -876,8 +908,11 @@ WMError WindowSceneSessionImpl::Show(uint32_t reason, bool withAnimation)
     if (state_ == WindowState::STATE_SHOWN) {
...
@@ -896,9 +931,8 @@ WMError WindowSceneSessionImpl::Show(uint32_t reason, bool withAnimation)
         UpdateDensity();
     }
...
@@ -906,11 +940,11 @@ WMError WindowSceneSessionImpl::Show(uint32_t reason, bool withAnimation)
         return ret;
     }
     UpdateTitleButtonVisibility();
-    if (WindowHelper::IsMainWindow(type)) {
...
@@ -940,7 +974,7 @@ WMError WindowSceneSessionImpl::Show(uint32_t reason, bool withAnimation)
...
@@ -974,14 +1008,15 @@ WMError WindowSceneSessionImpl::Hide(uint32_t reason, bool withAnimation, bool i
...
@@ -1139,8 +1174,9 @@ WMError WindowSceneSessionImpl::DestroyInner(bool needNotifyServer)
     }
...
+            ret = static_cast<WMError>(hostSession->Disconnect(true));
     ...
     return ret;
@@ -1212,7 +1248,10 @@ WMError WindowSceneSessionImpl::Destroy(bool needNotifyServer, bool needClearLis
         windowSessionMap_.erase(property_->GetWindowName());
 	}
...
+    }
     ...
     if (needClearListener) {
         ClearListenersById(GetPersistentId());
@@ -1259,7 +1298,9 @@ WMError WindowSceneSessionImpl::MoveTo(int32_t x, int32_t y)
...
+    auto hostSession = GetHostSession();
+    CHECK_HOST_SESSION_RETURN_ERROR_IF_NULL(hostSession, WMError::WM_ERROR_INVALID_OPERATION )
+    auto ret = hostSession->UpdateSessionRect(wsRect, SizeChangeReason::MOVE);
     return static_cast<WMError>(ret);
 }
 
@@ -1401,13 +1442,16 @@ WMError WindowSceneSessionImpl::Resize(uint32_t width, uint32_t height)
     property_->SetRequestRect(newRect);
...
@@ -1415,7 +1459,7 @@ WMError WindowSceneSessionImpl::SetAspectRatio(float ratio)
...
+    if (hostSession->SetAspectRatio(ratio) != WSError::WS_OK) {
     ...
     return WMError::WM_OK;
@@ -1423,11 +1467,9 @@ WMError WindowSceneSessionImpl::SetAspectRatio(float ratio)
...
@@ -1448,10 +1490,9 @@ WmErrorCode WindowSceneSessionImpl::RaiseToAppTop()
...
+    auto hostSession = GetHostSession();
+    CHECK_HOST_SESSION_RETURN_ERROR_IF_NULL(hostSession, WMError::WM_ERROR_STATE_ABNORMALLY);
+    const WSError& ret = hostSession->RaiseToAppTop();
     return static_cast<WmErrorCode>(ret);
 }
 
@@ -1483,24 +1524,21 @@ WmErrorCode WindowSceneSessionImpl::RaiseAboveTarget(int32_t subWindowId)
         WLOGFE("The sub window must be shown!");
         return WmErrorCode::WM_ERROR_STATE_ABNORMALLY;
     }
-    if (!hostSession_) {
...
@@ -1519,10 +1557,9 @@ WMError WindowSceneSessionImpl::NotifyWindowNeedAvoid(bool status)
...
@@ -1585,8 +1622,9 @@ WMError WindowSceneSessionImpl::SetLayoutFullScreen(bool status)
     }
...
@@ -1595,15 +1633,12 @@ WMError WindowSceneSessionImpl::SetLayoutFullScreen(bool status)
...
@@ -1763,15 +1798,13 @@ WMError WindowSceneSessionImpl::SetFullScreen(bool status)
...
@@ -1825,8 +1858,10 @@ WMError WindowSceneSessionImpl::Minimize()
         WLOGFE("session is invalid");
         return WMError::WM_ERROR_INVALID_WINDOW;
     }
...
+        hostSession->OnSessionEvent(SessionEvent::EVENT_MINIMIZE);
         ...
         return WMError::WM_DO_NOTHING;
@@ -1882,7 +1917,9 @@ WMError WindowSceneSessionImpl::MaximizeFloating()
...
+        hostSession->OnSessionEvent(SessionEvent::EVENT_MAXIMIZE_FLOATING);
         ...
         UpdateDecorEnable(true);
@@ -1900,13 +1937,15 @@ WMError WindowSceneSessionImpl::Recover()
         WLOGFE("session is invalid");
         return WMError::WM_ERROR_INVALID_WINDOW;
     }
-    if (WindowHelper::IsMainWindow(GetType()) && hostSession_) {
...
+        hostSession->OnSessionEvent(SessionEvent::EVENT_RECOVER);
         ...
         UpdateDecorEnable(true);
@@ -1933,13 +1972,15 @@ WMError WindowSceneSessionImpl::Recover(uint32_t reason)
...
@@ -1972,8 +2013,9 @@ void WindowSceneSessionImpl::StartMove()
     bool isFreeMutiWindowMode = windowSystemConfig_.freeMultiWindowSupport_ &&
         windowSystemConfig_.freeMultiWindowEnable_;
...
@@ -1997,10 +2039,12 @@ WMError WindowSceneSessionImpl::Close()
...
@@ -2014,7 +2058,9 @@ WMError WindowSceneSessionImpl::Close()
...
@@ -2102,14 +2148,15 @@ void WindowSceneSessionImpl::PerformBack()
...
@@ -2120,8 +2167,10 @@ WMError WindowSceneSessionImpl::SetGlobalMaximizeMode(MaximizeMode mode)
         WLOGFE("session is invalid");
         return WMError::WM_ERROR_INVALID_WINDOW;
     }
-    if (WindowHelper::IsMainWindow(GetType()) && hostSession_) {
...
@@ -2137,8 +2186,9 @@ MaximizeMode WindowSceneSessionImpl::GetGlobalMaximizeMode() const
...
@@ -2160,11 +2210,12 @@ WMError WindowSceneSessionImpl::SetWindowMode(WindowMode mode)
...
@@ -2401,8 +2452,10 @@ sptr<WindowSessionImpl> WindowSceneSessionImpl::GetWindowWithId(uint32_t winId)
...
@@ -2749,7 +2802,9 @@ WMError WindowSceneSessionImpl::UpdateSurfaceNodeAfterCustomAnimation(bool isAdd
...
@@ -2857,8 +2912,9 @@ WMError WindowSceneSessionImpl::SetCallingWindow(uint32_t callingSessionId)
...
+        hostSession->SetCallingSessionId(callingSessionId);
     ...
     return WMError::WM_OK;
@@ -3014,7 +3070,9 @@ WMError WindowSceneSessionImpl::NotifyPrepareClosePiPWindow()
...
@@ -3442,8 +3500,9 @@ WMError WindowSceneSessionImpl::AdjustKeyboardLayout(const KeyboardLayoutParams&
...
@@ -3451,7 +3510,7 @@ WMError WindowSceneSessionImpl::AdjustKeyboardLayout(const KeyboardLayoutParams&
...
index db4f1e59f42f3d2b70b0aa202fe8ccda0e669535..06bff63fa3cdb2190c5aa9e28cc36628bcc6649e 100644
...
-    hostSession_ = nullptr;
+    {
+        std::lock_guard<std::mutex> lock(hostSessionMutex)
+        hostSession = nullptr;
+    }
     {
         std::unique_lock<std::shared_mutex> lock(windowSessionMutex_);
...
+        return static_cast<WMError>(hostSession->SetKeyboardSessionGravity(
     ...
     return WMError::WM_OK;
@@ -3082,8 +3124,9 @@ void WindowSessionImpl::UpdatePiPRect(const Rect& rect, WindowSizeChangeReason r
         WLOGFE("HostSession is invalid");
         return;
     }
