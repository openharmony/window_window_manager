...
void PictureInPictureManager::DoScreenStatusChangeEvent(PiPScreenStatus status)
{
    TLOGI(WmsLogTag::WMS_PIP, "in");
    if (auto controller = GetScreenController()) {
        controller->ScreenStatusChange(status);
    }
}
