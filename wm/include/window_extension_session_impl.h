...
public:
    explicit WindowExtensionSessionImpl(const sptr<WindowOption>& option, sptr<IRemoteObject> renderSession = nullptr);
    ...
    WMError Destroy(bool needNotifyServer, bool needClearListener = true,
        uint32_t reason = static_cast<uint32_t>(WindowStateChangeReason::NORMAL),
    ...
    WMError GetWindowPropertyInfo(WindowPropertyInfo& windowPropertyInfoo, bool useHookedSize) override;
