...
WMError WindowManagerProxy::GetWindowStateSnapshot(int32_t persistentId, std::string& winStateSnapshotJsonStr)
{
    ...
    auto reqErrCode = remote->SendRequest(
        static_cast<uint32_t>(WindowManagerMessage::TRANS_ID_GET_WINDOW_STATE_SNAPSHOT),
        data, reply, option);
...
WMError WindowManagerProxy::NotifyScreenshotEvent(ScreenshotEventType type)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    if (!data.WriteInt32(static_cast<int32_t>(type))) {
        TLOGE(WmsLogTag::WMS_ATTRIBUTE, "Write screen shot event type failed");
        return WMError::WM_ERROR_IPC_FAILED;
    }
    sptr<IRemoteObject> remote = Remote();
    if (remote == nullptr) {
