...
public:
    ...
    {
        KeyboardAnimationCurve* config = new (std::nothrow)KeyboardAnimationCurve;
        if (config == nullptr) {
            return nullptr;
        }

        uint32_t paramSize = 0;
        if (!parcel.ReadString(config->curveType_) || !parcel.ReadUint32(paramSize) || paramSize != 4) { // 4:param size
            delete config;
            return nullptr;
        }
