...
#include "scenesessionmgrlstublayout_fuzzer.h"
...
void SceneSessionMgrLsLayoutTestCode(MessageParcel& parcel)
{
    MessageParcel reply;
    MessageOption option;
    parcel.RewindRead(0);
    SceneSessionManagerLite::GetInstance().OnRemoteRequest(static_cast<uint32_t>
        (ISceneSessionManagerLite::SceneSessionManagerLiteMessage::TRANS_ID_UPDATE_WINDOW_MODE_BY_ID_FOR_UI_TEST),
        parcel, reply, option);
}
...
bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
{
    ...
    SceneSessionMgrLsLayoutTestCode(parcel);
